...
rt_inline void imsic_id_clear_pending(rt_ubase_t id)
{
    imsic_eix_update(id, 1, RT_TRUE, RT_FALSE);
}
...
static rt_bool_t imsic_id_allocated(struct riscv_imsic *imsic, int hwirq)
{
    if (hwirq < IMSIC_ID_START || hwirq > imsic->global.nr_ids)
    {
        return RT_FALSE;
    }

    return rt_bitmap_test_bit(imsic->vectors, hwirq - IMSIC_ID_START);
}
...
static rt_err_t riscv_imsic_irq_init(struct rt_pic *pic)
{
    ...
    struct imsic_local_config *local = &imsic->global.local[cpu_id];
    ...
    imsic_local_delivery(RT_FALSE);

    for (int hwirq = IMSIC_ID_START; hwirq <= imsic->global.nr_ids; ++hwirq)
    {
        imsic_id_clear_enable(hwirq);
        imsic_id_clear_pending(hwirq);
    }

    for (int hwirq = IMSIC_ID_START; hwirq <= imsic->global.nr_ids; ++hwirq)
    {
        if (imsic_id_allocated(imsic, hwirq))
        {
...
static rt_err_t riscv_imsic_irq_set_affinity(struct rt_pic_irq *pirq, rt_bitmap_t *affinity)
{
    ...
    if (desc)
    {
        ...
        if (rt_bitmap_next_set_bit(affinity, cpu + 1, RT_CPUS_NR) != RT_CPUS_NR)
        {
            return -RT_ENOSYS;
        }

        if (!riscv_cpu_id_available(cpu) || !((struct riscv_imsic *)pirq->pic->priv_data)->global.local[cpu].online)
        {
            return -RT_EINVAL;
        }
...
static void riscv_imsic_irq_send_ipi(struct rt_pic_irq *pirq, rt_bitmap_t *cpumask)
{
    ...
    rt_bitmap_for_each_set_bit(cpumask, cpu_id, RT_CPUS_NR)
    {
        ...
        if (!riscv_cpu_id_available(cpu_id) || !local->online)
        {
            continue;
        }
...
static void riscv_imsic_irq_compose_msi_msg(struct rt_pic_irq *pirq, struct rt_pci_msi_msg *msg)
{
    ...
    cpu_id = rt_bitmap_next_set_bit(pirq->affinity, 0, RT_CPUS_NR);
    if (cpu_id >= RT_CPUS_NR || !riscv_cpu_id_available(cpu_id) || !global->local[cpu_id].online)
    {
        LOG_W("CPU%d MSI target is not available", cpu_id);
        return;
    }
...
static void riscv_imsic_irq_write_msi_msg(struct rt_pic_irq *pirq, struct rt_pci_msi_msg *msg)
{
    ...
    if (!imsic->parent_ic || !imsic->parent_ic->ops->irq_write_msi_msg)
    {
        return;
    }
...
static void imsic_handler(int hwirq, void *data)
{
    ...
    while ((local_id = csr_swap(CSR_TOPEI, 0)))
    {
        ...
        {
            if (!imsic_id_allocated(imsic, local_id))
            {
                imsic_id_clear_enable(local_id);
                LOG_W("Ignore unallocated IMSIC ID %d", local_id);
                continue;
            }
        ...
        if (!pirq || pirq->pic != &imsic->parent)
        {
            imsic_id_clear_enable(local_id);
            LOG_W("Ignore unmapped IMSIC ID %d", local_id);
            continue;
        }
...
{
    struct rt_pic *pic = rt_pic_dynamic_cast(rt_ofw_data(np));
    struct riscv_imsic *imsic = RT_NULL;

    if (pic && pic->ops == &riscv_imsic_ops)
    {
        imsic = rt_container_of(pic, struct riscv_imsic, parent);
    }
...
rt_bool_t imsic_ofw_is_available(void)
{
    struct rt_ofw_node *np;

    rt_ofw_foreach_node_by_ids(np, riscv_imsic_ofw_ids)
    {
        if (imsic_ofw_is_current_mode(np))
        {
            rt_ofw_node_put(np);
            return RT_TRUE;
        }
...
rt_bool_t imsic_ofw_is_current_mode(struct rt_ofw_node *np)
{
    struct rt_ofw_cell_args irq_args;
    ...
    return irq_args.args[0] == RV_IRQ_EXT;
}
        ...
        struct rt_ofw_node *ic_np, const struct rt_ofw_node_id *ic_id)
{
    ...
    rt_ofw_prop_read_u32(np, "riscv,group-index-bits", &global->group_index_bits);
    ...
    for (int i = 0; i < mmio_count; ++i)
    {
        ...
        if ((err = rt_ofw_get_address(np, i, &mmio[i].addr, &mmio[i].size)))
        {
            goto _fail;
        }
        ...
        mmio[i].mapped = rt_ofw_iomap(np, i);
    ...
    if (ic_np)
    {
        imsic->parent_ic = rt_pic_dynamic_cast(rt_ofw_data(ic_np));
    }
    ...
    if (!riscv_aclint_ipi_base)
    {
        for (int ipi = 0; ipi < RT_MAX_IPI; ++ipi)
        {
            ...
            rt_bitmap_set_bit(imsic->vectors, hwirq - IMSIC_ID_START);
...
_fail:
    if (mmio)
    {
        for (int i = 0; i < mmio_count && mmio[i].mapped; ++i)
        {
...
static rt_err_t imsic_ofw_init(struct rt_ofw_node *np, const struct rt_ofw_node_id *id)
{
    return imsic_ofw_probe(np, RT_NULL, id);
}
RT_PIC_OFW_DECLARE(imsic, riscv_imsic_ofw_ids, imsic_ofw_init);
