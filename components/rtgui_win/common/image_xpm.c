...
static rt_bool_t rtgui_image_xpm_load(rtgui_image_t* image, struct rtgui_filerw* file, rt_bool_t load);
...
struct rtgui_image_engine image_xpm_engine =
{
	"xpm",
	{RT_NULL},
...
static struct color_hash *CreateColorHash(int maxnum)
{
	int bytes, s;
	...
	hash->table = rt_malloc(bytes);
	if(!hash->table) return RT_NULL;

	rt_memset(hash->table, 0, bytes);
	hash->entries = rt_malloc(maxnum * sizeof(struct hash_entry));
...
static int add_colorhash(struct color_hash *hash, char *key, int cpp, rtgui_color_t *color)
{
...
static rt_bool_t rtgui_image_xpm_check(struct rtgui_filerw* file)
{
    ...
    if(rt_memcmp(buffer, "/* XPM */", (rt_uint32_t)XPM_MAGIC_LEN) == 0)
        result = RT_TRUE;
...
static rt_bool_t rtgui_image_xpm_load(rtgui_image_t* image, struct rtgui_filerw* file, rt_bool_t load)
{
	...
	/* build color table */
	colors_table = CreateColorHash(colors);
	if(!colors_table)
	{
		return RT_FALSE;
	}
...
color_none:
	...
	image->data = (rt_uint8_t*) rt_malloc(image->w * image->h * sizeof(rtgui_color_t));
...
{
	/* register xpm engine */
	rtgui_image_register_engine(&image_xpm_engine);
}
