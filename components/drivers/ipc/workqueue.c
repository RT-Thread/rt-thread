...
/* "Idle" here means no running work and no ready work; delayed work may still exist. */
...
/* Detach pending/delayed works, plus any requeued node of the current work. */
                                            ...
                                            rt_list_t *delayed_list)
{
        ...
        current->list.next != &(current->list))
    {
        ...
        /* Preserve CANCELING so sync waiters can still observe the in-flight state. */
        if (current->flags & RT_WORK_STATE_CANCELING)
        {
            current->flags = RT_WORK_STATE_CANCELING;
        }
        else
        {
            current->flags = RT_WORK_STATE_CANCELED;
        }
    }
...
/* The caller must exclude the currently running work from the detached list. */
static void _workqueue_cancel_detached_list(rt_list_t *list)
{
    ...
    while (!rt_list_isempty(list))
    {
        ...
        /* Detached work is no longer bound to any queue. */
        work->workqueue = RT_NULL;
    }
}

/*
 * Mark the queue destroying, detach queued works, and optionally register
 * the destroy_sync waiter state.
 */
                                           ...
                                           rt_list_t *delayed_list)
{
    ...
    queue_was_idle = _workqueue_is_idle_nolock(queue);
    if (sync)
    {
        /* destroy_sync reuses the sync-wait path until the worker exits. */
        queue->flags |= RT_WORKQUEUE_FLAG_SYNC_WAITING;
        if (queue->work_current != RT_NULL)
        {
            /* Prevent a running callback from requeueing itself past destroy_sync. */
            queue->work_current->flags |= RT_WORK_STATE_CANCELING;
        }
    }

    /* Reject new work immediately, then detach everything not currently running. */
    queue->flags |= RT_WORKQUEUE_FLAG_DESTROYING;
    _workqueue_detach_queued_nolock(queue, pending_list, delayed_list);
    rt_spin_unlock_irqrestore(&(queue->spinlock), level);

    /* Finish detached-list cancel outside the spinlock. */
    _workqueue_cancel_detached_list(pending_list);
    _workqueue_cancel_detached_list(delayed_list);
    if (queue_was_idle)
    {
        /* Wake a sleeping worker so it can observe destroying and exit. */
...
/* destroy() returns after marking destroying; destroy_sync() waits for drain. */
static rt_err_t _workqueue_destroy(struct rt_workqueue *queue, rt_bool_t sync)
{
    ...
    /* Wait until the worker drains the queue and hands ownership back. */
...
rt_inline rt_int32_t _workqueue_promote_delayed_nolock(struct rt_workqueue *queue)
{
    ...
    due_head = delayed_head->next;
    next_delayed = due_head;

    /* delayed_list is sorted; only the head prefix can be due. */
    while (next_delayed != delayed_head)
    {
        work = rt_list_entry(next_delayed, struct rt_work, list);
        /* Stop at the first timeout that is still in the future. */
        if ((current_tick - work->timeout_tick) < RT_TICK_MAX / 2)
        {
            work->flags = RT_WORK_STATE_PENDING;
            next_delayed = next_delayed->next;
        }
        else
        {
    ...
    if (next_delayed == due_head)
    {
        return delay_tick;
    }

    /* Append the due-prefix [due_head, due_tail] to work_list tail. */
    due_tail = next_delayed->prev;
    work_tail = work_head->prev;
    work_tail->next = due_head;
    due_head->prev = work_tail;
    due_tail->next = work_head;
    work_head->prev = due_tail;

    if (next_delayed == delayed_head)
    {
        /* The whole delayed list is due. */
        rt_list_init(delayed_head);
    }
    else
    {
        /* Keep the remaining delayed suffix linked to delayed_head. */
        delayed_head->next = next_delayed;
        next_delayed->prev = delayed_head;
    }

    return delay_tick;
}

/* Insert delayed work in timeout order. Equal timeout keeps FIFO order. */
rt_inline void _workqueue_insert_delayed_nolock(struct rt_workqueue *queue, struct rt_work *work)
{
    rt_list_t *delayed_list = &(queue->delayed_list);
    struct rt_work *head_work;
    struct rt_work *tail_work;
    struct rt_work *cursor;

    if (rt_list_isempty(delayed_list))
    {
        rt_list_insert_before(delayed_list, &(work->list));
        return;
    }

    /* Fast path: compare with head/tail to avoid full traversal. */
    head_work = rt_list_first_entry(delayed_list, struct rt_work, list);
    tail_work = rt_list_entry(delayed_list->prev, struct rt_work, list);

    /* Strictly earlier than the head goes to the front. */
    if (head_work->timeout_tick != work->timeout_tick &&
        (head_work->timeout_tick - work->timeout_tick) < RT_TICK_MAX / 2)
    {
        rt_list_insert_before(&(head_work->list), &(work->list));
        return;
    }

    /* Equal to, or later than, the tail can be appended directly. */
    if ((work->timeout_tick - tail_work->timeout_tick) < RT_TICK_MAX / 2)
    {
        rt_list_insert_before(delayed_list, &(work->list));
        return;
    }

    /* Find the first delayed work that should stay behind the new item. */
    rt_list_for_each_entry(cursor, delayed_list, list)
    {
        if (cursor->timeout_tick == work->timeout_tick)
        {
            /* Keep FIFO order for equal timeout. */
            continue;
        }
        if ((cursor->timeout_tick - work->timeout_tick) < RT_TICK_MAX / 2)
        {
            rt_list_insert_before(&(cursor->list), &(work->list));
            return;
        }
    }

    /* Fallback for wrap-around ordering: append at the end. */
    rt_list_insert_before(delayed_list, &(work->list));
}
...
rt_inline rt_bool_t _workqueue_cancel_queued_work_nolock(struct rt_workqueue *queue,
                                                         struct rt_work *work)
{
    rt_tick_t old_head_timeout = 0;
    rt_bool_t queue_was_idle = _workqueue_is_idle_nolock(queue);

    if (queue_was_idle && !rt_list_isempty(&(queue->delayed_list)))
    {
    ...
    {
        /* The worker may be waiting on the removed delayed head. */
        return RT_TRUE;
    }

    /* Wake only when the delayed head timeout changes after removal. */
    return rt_list_first_entry(&(queue->delayed_list), struct rt_work, list)->timeout_tick != old_head_timeout;
}
...
static void _workqueue_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        {
            notify_sync_waiter = (queue->flags & RT_WORKQUEUE_FLAG_SYNC_WAITING) != 0;
            rt_spin_unlock_irqrestore(&(queue->spinlock), level);

            if (notify_sync_waiter)
            {
        ...
        notify_sync_waiter = RT_FALSE;
        level = rt_spin_lock_irqsave(&(queue->spinlock));
        /* Drop the running marker before handling requeue and sync waiters. */
        queue->work_current = RT_NULL;
        /* A linked list node means the callback requeued this work. */
        requeued = work->list.next != &(work->list);
        /* cancel_work_sync wakes here; destroy_sync drains at queue exit. */
            ...
            !(queue->flags & RT_WORKQUEUE_FLAG_DESTROYING))
        {
            ...
            queue->flags &= ~RT_WORKQUEUE_FLAG_SYNC_WAITING;
            notify_sync_waiter = RT_TRUE;
        }
        if (!requeued)
        {
            /* Mark completion only when no requeue is pending. */
            work->flags = RT_WORK_STATE_DONE;
            /* Clear binding only after this work fully leaves this queue. */
            work->workqueue = RT_NULL;
        }
        rt_spin_unlock_irqrestore(&(queue->spinlock), level);
                                       ...
                                       rt_bool_t urgent)
{
    ...
    if (ticks == 0)
    {
        if (urgent)
        {
            /* Insert right after the list head so this work runs next. */
            rt_list_insert_after(&(queue->work_list), &(work->list));
        }
        else
        {
            /* Append at tail to keep FIFO order for normal submissions. */
            rt_list_insert_before(&(queue->work_list), &(work->list));
        }
        work->flags = RT_WORK_STATE_PENDING;
        /* Wake up when queue was sleeping for delayed work or new work. */
        need_wakeup = queue_was_idle;
    }
    else
    {
        rt_tick_t head_timeout;

        /* Enqueue delayed work (sorted by timeout). */
        /* Wake up only when the delayed wait point changes. */
        if (queue_was_idle)
        {
            if (rt_list_isempty(&(queue->delayed_list)))
            {
                /* Restart the worker so it switches from forever-wait to timed wait. */
                need_wakeup = RT_TRUE;
            }
            else
            {
                head_timeout = rt_list_first_entry(&(queue->delayed_list), struct rt_work, list)->timeout_tick;
                /* New work becomes the earliest delayed item. */
                need_wakeup = head_timeout != timeout_tick &&
                              (head_timeout - timeout_tick) < RT_TICK_MAX / 2;
            }
        }
 ...
 *       rt_workqueue_destroy_sync() if the caller must wait. When a work is
 *       still running, the queue enters destroying state and is freed by
 *       the worker thread after the queue drains.
 */
 ...
 *       RT_WORK_STATE_* is internal storage; this API maps it to the public
 *       RT_WORK_STATUS_* view and adds RUNNING / OTHER_QUEUE when applicable.
 */
rt_uint16_t rt_workqueue_get_work_status(struct rt_workqueue *queue, struct rt_work *work)
{
    ...
    {
        /* Bound elsewhere: report only the cross-queue relationship. */
        ...
        else
        {
            ...
            if (work->flags & RT_WORK_STATE_DONE)
            {
                status |= RT_WORK_STATUS_DONE;
            }
            if (work->flags & RT_WORK_STATE_CANCELED)
            {
    ...
    /* Bound to this queue: map internal runtime flags to the public status view. */
    if (work->flags & RT_WORK_STATE_PENDING)
    {
        status |= RT_WORK_STATUS_PENDING;
    }
    if (work->flags & RT_WORK_STATE_SUBMITTING)
    {
        status |= RT_WORK_STATUS_DELAYED;
    }
    if (work->flags & RT_WORK_STATE_CANCELING)
    {
        status |= RT_WORK_STATUS_CANCELING;
    }
    if (work->flags & RT_WORK_STATE_DONE)
    {
        status |= RT_WORK_STATUS_DONE;
    ...
    if (queue->work_current == work)
    {
        /* RUNNING is synthesized from queue state instead of work->flags. */
