...
//获取前导零的个数
inline U32 GD_INT_FFS(U32 value)
{
    if (value == 0)
        return value;
    ...
    //value &= (-value);
#if defined(_ARM_RVDS)
    __asm{clz value, value};
#elif defined(_ARM_GNU)
    __asm__ __volatile__  ("clz %0, %1": "=r"(value) :"r"(value));
#endif
...
{
    U32 irqStat=0,i=0,j=0;
    U32 value=0;
#if 1
    value = GD_INT_FFS(GH_VIC_get_IRQSts(0));
    if(value > 0)
    {
        j = value -1;
        GH_VIC_set_EdgeClr(0, 1 << j);
        if((ARM_INTR_IsrFunctionTable[j] != NULL))
            ARM_INTR_IsrFunctionTable[j]();
        return;
    }

    value = GD_INT_FFS(GH_VIC_get_IRQSts(1));
    if(value > 0)
    {
        j = value -1;
        GH_VIC_set_EdgeClr(1, 1 << j);
        if((ARM_INTR_IsrFunctionTable[GD_INT_VEC_OFFSET +j] != NULL))
            ARM_INTR_IsrFunctionTable[GD_INT_VEC_OFFSET +j]();
        return;
    }
    ...
    for(i = 0; i < GD_VIC_INSTANCES; i++)
    {
        ...
        if(irqStat == 0) continue;
        //value = GD_INT_FFS(irqStat);
        ...
        for(j = 0; j < 32; j++)
        {
            if((irqStat & (1 << j)))
            {
                ...
                if((ARM_INTR_IsrFunctionTable[i * GD_INT_VEC_OFFSET + j] != NULL))
...
void GD_FIQ_ISR(void)
{
    ...
    U32 value;
#if 1
        value = GD_INT_FFS(GH_VIC_get_FIQSts(0));
        if(value > 0)
        {
            j = value -1;
            GH_VIC_set_EdgeClr(0, 1 << j);
        ...
        value = GD_INT_FFS(GH_VIC_get_FIQSts(1));
        if(value > 0)
        {
            j = value -1;
            GH_VIC_set_EdgeClr(1, 1 << j);
