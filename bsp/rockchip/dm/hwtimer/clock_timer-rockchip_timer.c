...
static void rk_timer_init(struct rt_clock_timer_device *timer, rt_uint32_t state)
{
}

static rt_err_t rk_timer_start(struct rt_clock_timer_device *timer, rt_uint32_t cnt, rt_clock_timer_mode_t mode)
{
    rt_err_t err = RT_EOK;
...
static void rk_timer_stop(struct rt_clock_timer_device *timer)
{
    struct rk_timer *rk_timer = raw_to_rk_timer(timer);
...
static rt_uint32_t rk_timer_count_get(struct rt_clock_timer_device *timer)
{
    struct rk_timer *rk_timer = raw_to_rk_timer(timer);
...
static rt_err_t rk_timer_ctrl(struct rt_clock_timer_device *timer, rt_uint32_t cmd, void *args)
{
    rt_err_t err = RT_EOK;
    struct rk_timer *rk_timer = raw_to_rk_timer(timer);
    ...
    case CLOCK_TIMER_CTRL_MODE_SET:
        err = rk_timer_start(timer, rk_timer->cycle, (rt_clock_timer_mode_t)args);
        break;
...
const static struct rt_clock_timer_ops rk_timer_ops =
{
    .init = rk_timer_init,
