 ...
 * 2019-01-26     YLZ          add support EXTID RTR Frame. add filter for canopen.
 *                             modify send, recv functions and IRQs. fix bug.
 */

/* Includes ------------------------------------------------------------------*/
#include "drv_can.h"
//#define CANOPEN_SERVER
#ifdef CANOPEN_SERVER
/* user define get_ID function 0x600+ID */
__weak unsigned char get_ID(void)
{
    return 0;
}
#endif /* CANOPEN_SERVER */
...
static const struct stm_baud_rate_tab can_baud_rate_tab[] =
{
    ...
    {CAN20kBaud , (CAN_SJW_2TQ | CAN_BS1_11TQ | CAN_BS2_3TQ | 120)},
    {CAN10kBaud , (CAN_SJW_2TQ | CAN_BS1_11TQ | CAN_BS2_3TQ | 240)}
};
                              ...
                              struct can_configure *cfg)
{
    ...
    filterConf->FilterMaskIdLow = 0xFFF8;
    filterConf->FilterFIFOAssignment = CAN_FIFO0;
    filterConf->FilterActivation = ENABLE;
    filterConf->BankNumber = 14;
    HAL_CAN_ConfigFilter(&drv_can->CanHandle, filterConf);
    ...
    filterConf->FilterScale = CAN_FILTERSCALE_32BIT;
    filterConf->FilterIdHigh = ((0x600U + get_ID()) & 0x7FF) << 5;
    filterConf->FilterIdLow = 0x0000;
    filterConf->FilterMaskIdHigh = ((0x700U + get_ID()) & 0x7FF) << 5;;
    filterConf->FilterMaskIdLow = 0x0000;
    filterConf->FilterFIFOAssignment = CAN_FIFO0;
    filterConf->FilterActivation = ENABLE;
    filterConf->BankNumber = 14;
