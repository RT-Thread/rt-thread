...
GERR GD_PWM_Cycle(U8 channel, U32 highLevelCnt, U32 lowLevelCnt)
{
    ...
    if(mode==GD_NORMAL_SPEED)
    {
        ...
        pwm_control.bitc.highcnt = highLevelReg;
        pwm_control.bitc.lowcnt = lowLevelReg;
        GH_PWM_set_Control0(channel,pwm_control.all);
    }
    else if(mode==GD_SYNC_SPEED)
    {
        ...
        pwm_control.bitc.highcnt = highLevelReg;
        pwm_control.bitc.lowcnt = lowLevelReg;
        GH_PWM_set_Control1(channel,pwm_control.all);
    }
    return GD_OK;
}
...
{
    U32 mode;
    GH_PWM_CONTROL0_S pwm_control0;
    GH_PWM_CONTROL1_S pwm_control1;

    mode = GH_PWM_get_Mode_mode(pwm_duty->channel);
    if(mode == GD_NORMAL_SPEED)
    {
        pwm_control0.bitc.highcnt = pwm_duty->xon;
        pwm_control0.bitc.lowcnt  = pwm_duty->xoff;
        GH_PWM_set_Control0(pwm_duty->channel,pwm_control0.all);
    }
    else if(mode == GD_SYNC_SPEED)
    {
        pwm_control1.bitc.highcnt = pwm_duty->xon;
        pwm_control1.bitc.lowcnt  = pwm_duty->xoff;
...
GERR GD_PWM_Get_Duty(GD_PWM_DUTY_S *pwm_duty)
{
    ...
    if(mode == GD_NORMAL_SPEED)
    {
        pwm_control0.all = GH_PWM_get_Control0(pwm_duty->channel);
        pwm_duty->xon  = pwm_control0.bitc.highcnt;
        pwm_duty->xoff = pwm_control0.bitc.lowcnt;
    }
    else if(mode == GD_SYNC_SPEED)
    {
        pwm_control1.all = GH_PWM_get_Control1(pwm_duty->channel);
        pwm_duty->xon  = pwm_control1.bitc.highcnt;
        pwm_duty->xoff = pwm_control1.bitc.lowcnt;
    }
...
GD_PWM_ERROR_CODE_E GD_PWM_Set_Param(U8 channel, U32 frequency, U32 range, U32 duty)
{
    ...
    if(mode == GD_NORMAL_SPEED)
    {
        GH_PWM_set_Control0_HighCnt(channel, (highLevelCnt - 1));
        GH_PWM_set_Control0_LowCnt(channel, (lowLevelCnt - 1));
    }
    else
    {
        GH_PWM_set_Control1_HighCnt(channel, (highLevelCnt - 1));
        GH_PWM_set_Control1_LowCnt(channel, (lowLevelCnt - 1));
    }
...
GERR GD_PWM_Get_Param(U8 channel, U32 *frequency, U32 *duty)
{
    ...
    if(mode == GD_NORMAL_SPEED)
    {
        highLevelCnt = GH_PWM_get_Control0_HighCnt(channel) + 1;
        lowLevelCnt = GH_PWM_get_Control0_LowCnt(channel) + 1;
    }
    else
    {
        highLevelCnt = GH_PWM_get_Control1_HighCnt(channel) + 1;
        lowLevelCnt = GH_PWM_get_Control1_LowCnt(channel) + 1;
    }
