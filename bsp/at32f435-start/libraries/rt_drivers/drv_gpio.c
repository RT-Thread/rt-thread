...
static void at32_pin_mode(rt_device_t dev, rt_base_t pin, rt_base_t mode)
{
    ...
    gpio_init_struct.gpio_pins = PIN_ATPIN(pin);
    ...
    gpio_init(PIN_ATPORT(pin), &gpio_init_struct);
                                    ...
                                    rt_uint32_t mode, void (*hdr)(void *args), void *args)
{
    ...
    irqindex = bit2bitno(PIN_ATPIN(pin));
                                    ...
                                    rt_uint32_t enabled)
{
    ...
    if (enabled == PIN_IRQ_ENABLE)
    {
        ...
        gpio_init(PIN_ATPORT(pin), &gpio_init_struct);
        scfg_exint_line_config((scfg_port_source_type)PIN_ATPORTSOURCE(pin), (scfg_pins_source_type)PIN_ATPINSOURCE(pin));
    ...
    {
        irqmap = get_pin_irq_map(PIN_ATPIN(pin));
        if (irqmap == RT_NULL)
        {
            return -RT_EINVAL;
        }
...
#endif
    // RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_AFIO, TRUE);
