...
GERR GD_ETH_EPHY_Init(void)
{
    ...
    //WRREG(REG_EPHY_LDO, 0x00000000);//0x600223F8
    GH_EPHY_set_ADC_DC((U16)0x00000700);
    //WRREG(REG_EPHY_ADC, 0x00000700);//0x6002236C 
    GH_EPHY_set_PLL_ADC_CTRL0((U16)0x00001100);
    //WRREG(REG_EPHY_PLL_ADC_CTRL0,0x00001100);//0x60022394 
    GH_EPHY_set_PLL_ADC_CTRL1((U16)0x0000D180);
    //WRREG(REG_EPHY_PLL_ADC_CTRL1,0x0000D180);//0x60022398 
    GH_EPHY_set_PLL_ADC_CTRL2((U16)0x00000000);
    //WRREG(REG_EPHY_PLL_ADC_CTRL2,0x00000000);//0x600223A8 
    GH_EPHY_set_PLL_ADC_CTRL3((U16)0x00004000);
    //WRREG(REG_EPHY_PLL_ADC_CTRL3,0x00004000);//0x60022370
    GH_EPHY_set_RX_LPF((U16)0x00000500);
    //WRREG(REG_EPHY_RX_LPF,0x00000500);//0x60022374
    GH_EPHY_set_ADC_DC((U16)0x00000046);
    //WRREG(REG_EPHY_ADC_DC, 0x00000046);//0x600223D4
    GH_EPHY_set_MDIIO((U16)0x00000000);
    //WRREG(REG_EPHY_MDIIO,0x00000000);//0x60022540 
    GH_EPHY_set_POWER((U16)0x00003A03);
    //WRREG(REG_EPHY_POWER, 0x00003A03);  //0x60022474 power down close
    GH_EPHY_set_DEBUG((U16)0x0000C400);
    //WRREG(REG_EPHY_DEBUG, 0x0000C400);  //0x600220C8 reg_test_out(debug_bus_out)
    GH_EPHY_set_DEBUG_MODE((U16)0x0000810A);
    //WRREG(REG_EPHY_DEBUG_MODE, 0x0000810A);  //0x600220E0 debug mode
    // riu_cs_bkb
    //WRREG(0x60022588, RDREG(0x60022588)&(~(1<<14)));    // DAC 100M clk gate for 10M TX
    GH_EPHY_set_CLK0((U16)0x00000007);
    //WRREG(REG_EPHY_CLK0, 0x00000007); //0x60022588 DAC 100M clk gate for 10M TX
    ...
    GH_EPHY_set_CLK1((U16)0x00000043);
    //WRREG(REG_EPHY_CLK1, 0x0043);       //0x60022460 Add this 100M up down loop
    //GH_EPHY_set_SPEED_ane(0x01);
    ...
    WRREG(REG_EPHY_TRANSMIT_PATTER, RDREG(REG_EPHY_TRANSMIT_PATTER)|(1<<0));        //0x600221AC transmit pattern generation
#endif
...
GERR GD_ETH_EPHY_SetWorkMode(GD_HANDLE handle, GD_ETH_Work_ModeT workmode)
{
    ...
    else
    {
        ...
        {
            GH_EPHY_set_SPEED_duplex(GD_ETH_FULL_DUPLEX);
        }
        else if(workmode.duplex == GD_ETH_HALF_DUPLEX) /* half */
        {
            GH_EPHY_set_SPEED_duplex(GD_ETH_HALF_DUPLEX);
        }
        else
        {
            return GD_ERR_ETH_NOT_SUPPORTED;
...
{
    GH_EPHY_CONTROL_S   ctr;
    GH_EPHY_STATUS_S    status;
    ctr.all         = GH_EPHY_get_CONTROL();

    pStat->speed    = (ctr.bitc.mii_ctl_speed_sel_lsb) ? GD_ETH_SPEED_100M : GD_ETH_SPEED_10M;
    pStat->duplex   = (ctr.bitc.mii_ctl_duplex_mode) ? GD_ETH_FULL_DUPLEX : GD_ETH_HALF_DUPLEX;

    status.all      = GH_EPHY_get_STATUS();
    pStat->linkup   = (status.bitc.an_complete) ? GD_ETH_LINKUP : GD_ETH_LINKDOWN;
    return GD_OK;
}
