...
static void test_workqueue_submit_immediate(void)
{
    ...
    /* Keep the queue worker above the current test thread. */
...
static void test_workqueue_repeat_submit(void)
{
    ...
    work_flag = 0;
    rt_work_init(&work, wq_inc_work_fun, (void *)&work_flag);
    /* Repeat-submit regression for the same work item. */
    err = rt_workqueue_submit_work(queue, &work, 0);
    uassert_int_equal(err, RT_EOK);

    /* Phase: repeat submit while pending. */
    /* The same work should execute only once while it stays pending. */
    err = rt_workqueue_submit_work(queue, &work, 0);
    uassert_int_equal(err, RT_EOK);
    rt_thread_delay(rt_tick_from_millisecond(10));
    /* Confirm the pending duplicate was coalesced. */
    uassert_int_equal(work_flag, 1);

    rt_thread_delay(rt_tick_from_millisecond(100));
    rt_workqueue_destroy(queue);
    /* Phase: repeat submit while running. */
    curr_priority = wq_get_test_thread_priority(-1);
    ...
    /* Submit work with higher queue priority so it runs immediately. */
    err = rt_workqueue_submit_work(queue, &work, 0);
    uassert_int_equal(err, RT_EOK);

    rt_thread_delay(rt_tick_from_millisecond(5));
    /* Repeat submit while running should queue one more execution. */
    err = rt_workqueue_submit_work(queue, &work, 0);
    uassert_int_equal(err, RT_EOK);
    rt_thread_delay(rt_tick_from_millisecond(10));
    uassert_int_equal(work_flag, 1);
...
static void test_workqueue_repeat_requeue_limit(void)
{
    ...
    /* Wait for the work to be executed 20 times. */
    err = rt_sem_take(&ctx.done, rt_tick_from_millisecond(500));
    uassert_int_equal(err, RT_EOK);

    /* Confirm the work ran 20 times. */
...
static void test_workqueue_repeat_delay(void)
{
    ...
    /* Keep the queue worker below the current test thread. */
    curr_priority = wq_get_test_thread_priority(1);
    queue = rt_workqueue_create("test", 2048, curr_priority);
    uassert_not_null(queue);
    if (queue == RT_NULL)
    ...
    /* Phase: delayed repeat before run. */
    /* The delayed work should still be pending here. */
    rt_thread_delay(rt_tick_from_millisecond(10));
    /* Repeat submit of delayed work before it runs. */
    err = rt_workqueue_submit_work(queue, &work, rt_tick_from_millisecond(20));
    uassert_int_equal(err, RT_EOK);
    ...
    /* Phase: delayed repeat after run. */
    work_flag = 0;
    rt_work_init(&work, wq_inc_delay_work_fun, (void *)&work_flag);
    ...
    /* Wait for the delayed work to execute. */
    rt_thread_delay(rt_tick_from_millisecond(25));
    err = rt_workqueue_submit_work(queue, &work, rt_tick_from_millisecond(20));
    uassert_int_equal(err, RT_EOK);

    /* Confirm the delayed work ran once before resubmission. */
    rt_thread_delay(rt_tick_from_millisecond(10));
    uassert_int_equal(work_flag, 1);

    rt_thread_delay(rt_tick_from_millisecond(25));
    /* Confirm the delayed work ran twice after resubmission. */
    uassert_int_equal(work_flag, 2);
...
void workqueue_basic_testcase(void)
{
    ...
    /* Urgent on delayed work runs first. */
    UTEST_UNIT_RUN(test_workqueue_urgent_delayed);
    /* Delayed and immediate reschedule. */
    ...
    /* Repeat delayed submit before execution. */
