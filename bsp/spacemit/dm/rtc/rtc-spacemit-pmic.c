...
#define SPACEMIT_PMIC_RTC_READ_RETRIES  10
#define SPACEMIT_PMIC_RTC_WRITE_RETRIES 10
...
static rt_err_t spacemit_pmic_rtc_read_time(struct spacemit_pmic_rtc *pmic_rtc, time_t *sec)
{
    ...
    for (int retries = 0; retries < SPACEMIT_PMIC_RTC_READ_RETRIES; ++retries)
    {
    ...
    if (!stable)
    {
        LOG_E("RTC counter did not stabilize");
        return -RT_ETIMEOUT;
    }
...
{
    rt_err_t err;
    rt_bool_t verified = RT_FALSE;
    unsigned int v[6];
    struct tm tm_buf, *tm;
    union spacemit_pmic_rtc_ctl_desc rtc_ctl;
    struct spacemit_pmic *pmic = pmic_rtc->pmic;

    tm = localtime_r(sec, &tm_buf);
    ...
    for (int retries = 0; retries < SPACEMIT_PMIC_RTC_WRITE_RETRIES; ++retries)
    {
    ...
    if (!verified)
    {
        LOG_E("RTC time write verification timed out");
        return -RT_ETIMEOUT;
    }
