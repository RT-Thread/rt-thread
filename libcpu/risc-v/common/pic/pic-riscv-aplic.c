...
static rt_err_t aplic_direct_irq_set_priority(struct rt_pic_irq *pirq, rt_uint32_t priority)
{
    ...
    priority = rt_min_t(rt_uint32_t, priority,
            APLIC_TARGET_IPRIO_MASK - APLIC_DEFAULT_PRIORITY);
...
{
    int cpu_id;
    rt_uint32_t priority, value;
    struct riscv_aplic *aplic = pirq->pic->priv_data;

    cpu_id = rt_bitmap_next_set_bit(affinity, 0, RT_CPUS_NR);
    if (cpu_id >= RT_CPUS_NR || !riscv_cpu_id_available(cpu_id) || !aplic->idc[cpu_id].valid)
    {
        return -RT_EINVAL;
    }

    priority = rt_min_t(rt_uint32_t, pirq->priority,
            APLIC_TARGET_IPRIO_MASK - APLIC_DEFAULT_PRIORITY);
    priority += APLIC_DEFAULT_PRIORITY;

    value = RT_FIELD_PREP(APLIC_TARGET_HART_IDX, aplic->idc[cpu_id].hart_index);
    value |= RT_FIELD_PREP(APLIC_TARGET_IPRIO, priority);
...
static rt_err_t aplic_msi_irq_set_affinity(struct rt_pic_irq *pirq, rt_bitmap_t *affinity)
{
    ...
    cpu_id = rt_bitmap_next_set_bit(affinity, 0, RT_CPUS_NR);
    if (cpu_id >= RT_CPUS_NR || !riscv_cpu_id_available(cpu_id) ||
        !aplic->imsic_global || !aplic->imsic_global->local[cpu_id].online)
    {
        return -RT_EINVAL;
    }

    local = &aplic->imsic_global->local[cpu_id];

    value = RT_FIELD_PREP(APLIC_TARGET_HART_IDX, local->hart_index);
...
static int aplic_irq_map(struct rt_pic *pic, int hwirq, rt_uint32_t mode)
{
    ...
    if (hwirq <= 0 || hwirq > aplic->nr_irqs)
    {
        return -RT_EINVAL;
    }
...
static rt_err_t aplic_irq_parse(struct rt_pic *pic, struct rt_ofw_cell_args *args, struct rt_pic_irq *out_pirq)
{
    ...
    if (args->args_count < 2 || !args->args[0] || args->args[0] > aplic->nr_irqs)
    {
        return -RT_EINVAL;
    }
...
static rt_err_t aplic_ofw_init(struct rt_ofw_node *np, const struct rt_ofw_node_id *id)
{
    rt_err_t err;
    rt_uint32_t val;
    struct rt_ofw_node *msi_np = RT_NULL;
    struct riscv_aplic *aplic;

    msi_np = rt_ofw_parse_phandle(np, "msi-parent", 0);

    if (msi_np)
    {
        if (!imsic_ofw_is_current_mode(msi_np))
        {
    ...
    else
    {
        ...
        if (rt_ofw_parse_irq_cells(np, 0, &irq_args) ||
            irq_args.args_count < 1 || irq_args.args[0] != RV_IRQ_EXT)
        {
            return RT_EOK;
        }
    }

    if (!(aplic = rt_calloc(1, sizeof(*aplic))))
    {
        err = -RT_ENOMEM;
        goto _fail;
    }
    ...
    if (!aplic->nr_irqs || aplic->nr_irqs > APLIC_MAX_SOURCE)
    {
        err = -RT_EINVAL;
        goto _fail;
    }
    ...
    if (msi_np)
    {
        ...
        aplic->imsic_global = imsic_global;
    ...
    {
        int hwirq, cpu_id, idc_count;
        rt_ubase_t hartid;
        struct riscv_aplic_idc *idc;
        struct rt_ofw_cell_args idcs;
        struct rt_ofw_node *cpu_np;

        idc_count = rt_min_t(int, rt_ofw_get_irq_count(np), APLIC_MAX_IDC);

        for (int idc_index = 0; idc_index < idc_count; ++idc_index)
        {
            if (rt_ofw_parse_irq_cells(np, idc_index, &idcs) || idcs.args_count < 1)
            {
                continue;
            }
            ...
            idc->regs = aplic->regs + APLIC_IDC_BASE + idc_index * APLIC_IDC_SIZE;
            idc->hart_index = idc_index;
            idc->valid = RT_TRUE;
    ...
    if (msi_np)
    {
        ...
        aplic->msi_ic = rt_pic_dynamic_cast(rt_ofw_data(msi_np));
    }
    else
    {
    ...
    /* APLIC source identities are 1-based; keep index 0 reserved. */
    rt_pic_linear_irq(&aplic->parent, aplic->nr_irqs + 1);
...
_fail:
    if (aplic && rt_ofw_data(np) == &aplic->parent)
    {
