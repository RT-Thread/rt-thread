...
rt_err_t rt_sem_detach(rt_sem_t sem)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(sem->spinlock));
    /* wakeup all suspended threads */
    _ipc_list_resume_all(&(sem->parent.suspend_thread));

    /* detach semaphore object */
    rt_object_detach(&(sem->parent.parent));
    rt_arch_spin_unlock_irqrestore(&(sem->spinlock), level);
...
rt_uint8_t rt_mutex_setprioceiling(rt_mutex_t mutex, rt_uint8_t priority)
{
    ...
    if ((mutex) && (priority < RT_THREAD_PRIORITY_MAX))
    {
        ...
        rt_ubase_t level = rt_arch_spin_lock_irqsave(&(mutex->spinlock));
...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
    ...
    if (mutex->hold == 0)
    {
        ...
        if (!rt_list_isempty(&mutex->parent.suspend_thread))
        {
            ...
            rt_arch_spin_lock(&(next_thread->spinlock));
            ...
            rt_arch_spin_unlock(&(next_thread->spinlock));
            /* resume thread */
...
rt_err_t rt_event_detach(rt_event_t event)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(event->spinlock));
    /* resume all suspended thread */
    _ipc_list_resume_all(&(event->parent.suspend_thread));

    /* detach event object */
    rt_object_detach(&(event->parent.parent));
    rt_arch_spin_unlock_irqrestore(&(event->spinlock), level);
...
rt_err_t rt_event_delete(rt_event_t event)
{
    ...
    rt_arch_spin_lock(&(event->spinlock));
    /* resume all suspended thread */
    _ipc_list_resume_all(&(event->parent.suspend_thread));

    /* delete event object */
    rt_object_delete(&(event->parent.parent));
    rt_arch_spin_unlock(&(event->spinlock));
...
rt_err_t rt_mb_detach(rt_mailbox_t mb)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(mb->spinlock));
    /* resume all suspended thread */
    ...
    rt_arch_spin_unlock_irqrestore(&(mb->spinlock), level);
...
rt_err_t rt_mb_delete(rt_mailbox_t mb)
{
    ...
    rt_arch_spin_unlock(&(mb->spinlock));
...
rt_err_t rt_mq_detach(rt_mq_t mq)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(mq->spinlock));
    /* resume all suspended thread */
    ...
    rt_arch_spin_unlock_irqrestore(&(mq->spinlock), level);
...
rt_err_t rt_mq_delete(rt_mq_t mq)
{
    ...
    rt_arch_spin_lock(&(mq->spinlock));
    /* resume all suspended thread */
    _ipc_list_resume_all(&(mq->parent.suspend_thread));
    /* also resume all message queue private suspended thread */
    _ipc_list_resume_all(&(mq->suspend_sender_thread));

    rt_arch_spin_unlock(&(mq->spinlock));
