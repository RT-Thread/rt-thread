...
static struct at32_clock_timer at32_clock_timer_obj[] =
{
#ifdef BSP_USING_HWTMR1
    TMR1_CONFIG,
#endif
...
static void at32_timer_init(struct rt_clock_timer_device *timer, rt_uint32_t state)
{
    crm_clocks_freq_type clocks_struct;
    rt_uint32_t pclk1_doubler = 0, pclk2_doubler = 0;
    rt_uint32_t prescaler_value = 0, tmr_clock = 0;
    tmr_type *tmr_x = RT_NULL;
    struct at32_clock_timer *tmr_device = RT_NULL;

    RT_ASSERT(timer != RT_NULL);
    if (state)
    {
        tmr_x = (tmr_type *)timer->parent.user_data;
        tmr_device = (struct at32_clock_timer *)timer;
...
static rt_err_t at32_timer_start(rt_clock_timer_t *timer, rt_uint32_t pr, rt_clock_timer_mode_t opmode)
{
...
static void at32_timer_stop(rt_clock_timer_t *timer)
{
    tmr_type *tmr_x = RT_NULL;
...
static rt_uint32_t at32_timer_counter_get(rt_clock_timer_t *timer)
{
    tmr_type *tmr_x = RT_NULL;
...
static rt_err_t at32_timer_ctrl(rt_clock_timer_t *timer, rt_uint32_t cmd, void *arg)
{
    crm_clocks_freq_type clocks_struct;
    tmr_type *tmr_x = RT_NULL;
    rt_err_t result = RT_EOK;
...
void TMR2_GLOBAL_IRQHandler(void)
{
    ...
    {
        rt_clock_timer_isr(&at32_clock_timer_obj[TMR2_INDEX].tmr_device);
...
void TMR3_GLOBAL_IRQHandler(void)
{
    ...
    {
        rt_clock_timer_isr(&at32_clock_timer_obj[TMR3_INDEX].tmr_device);
...
void TMR4_GLOBAL_IRQHandler(void)
{
    ...
    {
        rt_clock_timer_isr(&at32_clock_timer_obj[TMR4_INDEX].tmr_device);
...
static int rt_hw_clock_timer_init(void)
{
    ...
    for (i = 0; i < sizeof(at32_clock_timer_obj) / sizeof(at32_clock_timer_obj[0]); i++)
    {
        at32_clock_timer_obj[i].tmr_device.info = &_info;
        at32_clock_timer_obj[i].tmr_device.ops  = &_ops;
        if (rt_clock_timer_register(&at32_clock_timer_obj[i].tmr_device, at32_clock_timer_obj[i].name, at32_clock_timer_obj[i].tmr_x) == RT_EOK)
        {
            LOG_D("%s register success", at32_clock_timer_obj[i].name);
        }
        else
        {
            LOG_E("%s register failed", at32_clock_timer_obj[i].name);
            result = -RT_ERROR;
        }
    }
