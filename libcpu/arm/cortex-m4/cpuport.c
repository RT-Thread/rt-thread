...
void rt_hw_stack_guard_init(rt_thread_t thread)
{
    ...
    rt_ubase_t stack_top_region_start = RT_ALIGN_DOWN(stack_bottom, MPU_MIN_REGION_SIZE);
    ...
    thread->stack_addr = (void *)(stack_bottom_region_start + MPU_MIN_REGION_SIZE - 1);
    thread->stack_size -= stack_bottom_region_start - stack_bottom;
    thread->stack_size -= stack_top - stack_top_region_start;
}
#endif
