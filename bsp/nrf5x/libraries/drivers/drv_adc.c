 ...
 * Change Logs:
 * Date           Author       Notes
 * 2020-08-16     guohp1128    the first version
 */
...
div_nrfx_saadc_result_t results;
...
static void set_channels(div_nrfx_saadc_channel_t * channel)
{
    uint8_t i;
...
{      
    ...
    { 
        ...
        div_nrfx_saadc_channel_t * drv_channel_config = NULL;
        drv_channel_config = (div_nrfx_saadc_channel_t *)device->parent.user_data;   
...
int rt_hw_adc_init(void)
{
    ...
    if (nrfx_saadc_init(NRFX_SAADC_CONFIG_IRQ_PRIORITY) != NRFX_SUCCESS)
    {
        ...
        LOG_E("The driver is already initialized.");
        result = -RT_ERROR;
    }
    else
    {
...
{
    div_nrfx_saadc_channel_t channel_config;
    rt_uint32_t result; 
    ...
    channel_config = (div_nrfx_saadc_channel_t){.mode = 0, .pin_p = 1, .pin_n = 1,.channel_num = 0};
    rt_adc_enable(adc_dev, channel_config.channel_num);
        
    channel_config = (div_nrfx_saadc_channel_t){.mode = 0, .pin_p = 2, .pin_n = 1,.channel_num = 1};
    rt_adc_enable(adc_dev, channel_config.channel_num);
    
    channel_config = (div_nrfx_saadc_channel_t){.mode = 0, .pin_p = 7, .pin_n = 1,.channel_num = 5};
    rt_adc_enable(adc_dev, channel_config.channel_num);
