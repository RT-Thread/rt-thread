...
U8 *g_buffer=NULL;
U32 g_block_size=0;
U32 resp[SDIO_HANDLE_COUNT][4]={0};
...
static sdioHandleT    sdioHandleArray[SDIO_HANDLE_COUNT] = {0};
...
{
    U32 modereg=GH_SDIO_get_TranModeNorIntSigEnReg(index);
...
static int GD_SDIO_GetScr(sdioBlockT *cardInfo)
{
    ...
    {
        ret = sdioIssueCmd(cardInfo->index, SD_APP_SEND_SCR,0,0, MMC_RSP_R1);
        if(ret)
        {
            return -1;
        }

        cardInfo->scr.scrStructure  =  2;
        cardInfo->scr.sdSpec        =  2;
        cardInfo->scr.stateaftererase    =  2;
        cardInfo->scr.sdSecurity    =  3;
        cardInfo->scr.sdBusWith     =  5;
    }
#endif
...
static int GD_SDIO_WriteBlocks(sdioBlockT *cardInfo, U32 isusedma,void *scr, U32  start,U32  blkcnt)
{
    ...
    if(cmdidx==MMC_WRITE_MULTIPLE_BLOCK)
    {
        ...
        {
            if(sdioIssueCmd(cardInfo->index,23,blkcnt,0,MMC_RSP_R1))
                return -1;
        }
    }
#endif
...
static GISR1 GD_SDIO_ISR0(void)
{
    ...
    if( irq_status_reg & SDIO_ERROR_IRQ )
    {
        ...
        GH_SDIO_set_ErrIntStaEnNorIntStaReg(index,GH_SDIO_get_ErrIntStaEnNorIntStaReg(index));
        *(volatile U16 *)(0x90000000+0x14) = *(volatile U16 *)(0x90000000+0x14);
        GH_SDIO_set_Control01Reg_SoftwareResetDatLine(index,1);
        GH_SDIO_set_Control01Reg_SoftwareResetCmdLine(index,1);
