...
GERR GD_USB_DMA_In_TranRx(U32 host_write_addr, U32 host_write_len)
{
    ...
    GH_DMA_set_Source_Address(MUSB_RX_DMA_CHAN, REG_USB_ADDR_FIFO_EP(g_rx_ep));    //USB FIFO addr
...
{
    GH_USB_set_Index(g_tx_ep);   //EP1 in
    GH_USB_set_TXCSRH_W_PERI(0xb4);
    GH_DMA_set_Source_Address(MUSB_TX_DMA_CHAN, read_addr);                //source data addr
    GH_DMA_set_Destination_Address(MUSB_TX_DMA_CHAN, REG_USB_ADDR_FIFO_EP(g_tx_ep));  //USB FIFO addr
