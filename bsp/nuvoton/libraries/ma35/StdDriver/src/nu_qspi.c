...
static uint32_t QSPI_GetSrcClkSetting(QSPI_T *qspi)
{
    uint32_t u32ClkSrcFreq = 0UL, u32ClkSrcSel;
    uint32_t u32ClkSelMsk = 0U, u32ClkSelPos = 0U;

    if (qspi == QSPI0)
    {
        u32ClkSelMsk = CLK_CLKSEL4_QSPI0SEL_Msk;
        u32ClkSelPos = CLK_CLKSEL4_QSPI0SEL_Pos;
    }
    else if (qspi == QSPI1)
    {
        u32ClkSelMsk = CLK_CLKSEL4_QSPI1SEL_Msk;
        u32ClkSelPos = CLK_CLKSEL4_QSPI1SEL_Pos;
    }
    else
    {
        goto Exit_QSPI_GetSrcClkSetting;
    }

    u32ClkSrcSel = (CLK->CLKSEL4 & u32ClkSelMsk) >> u32ClkSelPos;

    switch (u32ClkSrcSel)
    {
    case 0x0U:
        /* Clock source is PCLK0 */
        u32ClkSrcFreq = CLK_GetPCLK0Freq();
        break;
    case 0x1U:
        /* Clock source is APLL */
        u32ClkSrcFreq = CLK_GetPLLClockFreq(APLL);
        break;
    default:
        break;
    }

Exit_QSPI_GetSrcClkSetting:

    return u32ClkSrcFreq;
}
...
static void QSPI_SetSrcClkToPCLK(QSPI_T *qspi)
{
    ...
    CLK->CLKSEL4 = (CLK->CLKSEL4 & (~u32ClkSelMsk)) | (0x2UL << u32ClkSelPos);
}
...
{
    uint32_t u32ClkSrcFreq = 0U, u32Div, u32HCLKFreq, u32RetValue = 0U;
    ...
    if (u32MasterSlave == QSPI_MASTER)
    {
        ...
        {
            /* Select PCLK as the clock source of QSPI */
            QSPI_SetSrcClkToPCLK(qspi);
        }

        /* Check clock source of QSPI */
        u32ClkSrcFreq = QSPI_GetSrcClkSetting(qspi);

        if ((u32BusClock >= u32HCLKFreq) || (u32BusClock >= u32ClkSrcFreq))
        {
            /* Set DIVIDER = 0 */
            qspi->CLKDIV = 0U;
            /* Return master peripheral clock rate */
            u32RetValue = u32ClkSrcFreq;
        }
        else if (u32BusClock == 0U)
        {
            ...
            /* Return master peripheral clock rate */
            u32RetValue = (u32ClkSrcFreq / (0xFFU + 1U));
        }
        else
        {
            u32Div = (((u32ClkSrcFreq * 10U) / u32BusClock + 5U) / 10U) - 1U; /* Round to the nearest integer */
            if (u32Div > 0xFFU)
            {
                u32Div = 0xFFU;
                qspi->CLKDIV |= QSPI_CLKDIV_DIVIDER_Msk;
            ...
            else
            {
                ...
                /* Return master peripheral clock rate */
                u32RetValue = (u32ClkSrcFreq / (u32Div + 1U));
            }
        }
    }
...
{
    uint32_t u32ClkSrcFreq, u32HCLKFreq;
    ...
    else if (u32BusClock >= u32ClkSrcFreq)
    {
        /* Set DIVIDER = 0 */
        qspi->CLKDIV = 0U;
        /* Return master peripheral clock rate */
