...
static rt_bool_t show_demo(struct rtgui_win *win)
{
    ...
    /* draw image */
    {
        ...
        img = rtgui_image_create_from_file("png", "/mali.png", RT_TRUE);
        // img = rtgui_image_create_from_file("bmp", "/mali.bmp", RT_TRUE);
        // img = rtgui_image_create_from_file("jpg", "/1.jpg", RT_TRUE);
        
        // img = rtgui_image_create_from_file("png", "/4.png", RT_TRUE);
        // img = rtgui_image_create_from_file("jpg", "/1.bmp", RT_TRUE);
        if (img != RT_NULL)
        {
        ...
        {
            rt_kprintf("image created failed!\n");
        }
    }
    ...
    //     rtgui_color_t fc;
    //     rtgui_rect_t draw_rect;
    //     char *text_buf = "测试长长中文。。。。";

    //     fc = RTGUI_DC_FC(dc);
    //     RTGUI_DC_FC(dc) = RED;
    //     draw_rect.x1 = rect.x2 * 1 / 5;
