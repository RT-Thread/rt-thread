 ...
 * - Destroy returns immediately while running work drains asynchronously.
 ...
 * - Destroy-sync waits for running work to finish.
 * - Destroy returns before blocked running work is released.
 * - Destroy returns busy when sync waiter or destroying flag exists.
...
static void wq_destroy_async_entry(void *parameter)
{
    struct wq_destroy_wait_ctx *ctx = (struct wq_destroy_wait_ctx *)parameter;
...
static void wq_destroy_status_work_fun(struct rt_work *work, void *work_data)
{
    ...
    /* Requeue before destroy_sync marks canceling to reproduce the status transition. */
...
static void test_workqueue_destroy_sync_behaviors(void)
{
    ...
    /* Destroy sync should wait for the running work to finish. */
    err = rt_workqueue_destroy_sync(queue);
    uassert_int_equal(err, RT_EOK);
...
static void test_workqueue_destroy_async_while_running(void)
{
    struct rt_workqueue *queue;
    rt_uint8_t curr_priority;
    struct rt_work work;
    ...
    err = rt_sem_init(&run_ctx.started, "wqas", 0, RT_IPC_FLAG_FIFO);
    uassert_int_equal(err, RT_EOK);
    err = rt_sem_init(&run_ctx.allow_finish, "wqaf", 0, RT_IPC_FLAG_FIFO);
    uassert_int_equal(err, RT_EOK);
    run_ctx.phase = 1;
    run_ctx.count = 0;
    run_ctx.released = 0;
    ...
    err = rt_sem_init(&destroy_ctx.started, "wqaz", 0, RT_IPC_FLAG_FIFO);
    uassert_int_equal(err, RT_EOK);
    err = rt_sem_init(&destroy_ctx.done, "wqad", 0, RT_IPC_FLAG_FIFO);
    uassert_int_equal(err, RT_EOK);
    destroy_ctx.queue = queue;
    destroy_ctx.err = RT_EOK;

    destroy_thread = rt_thread_create("wqadt", wq_destroy_async_entry, &destroy_ctx,
                                      1024, curr_priority, 10);
    ...
    uassert_int_equal(run_ctx.done, 0);

    /* Destroy returns immediately; the running work still drains asynchronously. */
    rt_sem_release(&run_ctx.allow_finish);
    ...
    uassert_int_equal(run_ctx.done, 1);
    uassert_int_equal(run_ctx.count, 1);
...
static void test_workqueue_destroy_busy_when_sync_waiting(void)
{
    ...
    /* Destroy should return busy while a sync waiter exists. */
    err = rt_workqueue_destroy(queue);
    uassert_int_equal(err, -RT_EBUSY);
...
static void test_workqueue_destroy_busy_when_destroying(void)
{
    ...
    /* Simulate destroying flag and ensure destroy returns busy. */
    level = rt_spin_lock_irqsave(&(queue->spinlock));
...
void workqueue_destroy_testcase(void)
{
    ...
    /* Destroy should return before a blocked running work is released. */
    UTEST_UNIT_RUN(test_workqueue_destroy_async_while_running);
    /* Destroy returns busy when a sync waiter exists. */
    UTEST_UNIT_RUN(test_workqueue_destroy_busy_when_sync_waiting);
    /* Destroy returns busy when destroying flag is already set. */
    UTEST_UNIT_RUN(test_workqueue_destroy_busy_when_destroying);
    /* Destroying queue rejects submit and urgent. */
    UTEST_UNIT_RUN(test_workqueue_destroying_reject_submit_urgent);
    /* Running work should stay CANCELING, not CANCELED. */
