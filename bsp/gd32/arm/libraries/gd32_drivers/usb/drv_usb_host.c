...
usb_status usb_host_init(usb_core_driver *udev)
{
    ...
    *udev->regs.PWRCLKCTL = 0U;
    ...
    /* set RX FIFO size */
    udev->regs.gr->GRFLEN = USB_HOST_RX_FIFO_SIZE;

    /* set non-periodic TX FIFO size and address */
    nptxfifolen |= USB_HOST_RX_FIFO_SIZE;
    nptxfifolen |= USB_HOST_TX_NPFIFO_SIZE << 16;
    udev->regs.gr->DIEP0TFLEN_HNPTFLEN = nptxfifolen;

    /* set periodic TX FIFO size and address */
    ptxfifolen |= USB_HOST_RX_FIFO_SIZE + USB_HOST_TX_PFIFO_SIZE;
    ptxfifolen |= USB_HOST_TX_PFIFO_SIZE << 16;
