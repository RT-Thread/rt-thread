        ...
        struct k1x_thermal_sensor_desc *desc)
{
    ...
    {
        struct sensor_enable *se = &desc[i].se;

        temp = HWREG32(thermal->regs + se->bjt_en);
        temp &= ~se->bit_msk;
        temp |= se->en_val << se->offset;
        HWREG32(thermal->regs + se->bjt_en) = temp;
    }
...
static rt_err_t k1x_thermal_probe(struct rt_platform_device *pdev)
{
    ...
    thermal->sensors = rt_calloc(thermal->sr[1] - thermal->sr[0] + 1, sizeof(*thermal->sensors));
    if (!thermal->sensors)
    {
        err = -RT_ENOMEM;
        goto _fail;
