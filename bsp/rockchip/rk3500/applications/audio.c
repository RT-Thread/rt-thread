 ...
 * Change Logs:
 * Date           Author       Notes
 * 2026-07-22     GuEe-GUI     first version
 */

#include <rtthread.h>
#include <rtdevice.h>

#if defined(RT_USING_AUDIO) && defined(RT_AUDIO_ROCKCHIP) && defined(RT_USING_FINSH)
#include <finsh.h>
#include <stdlib.h>

#define AUDIO_TEST_DEVICE           "rockchip-hdmi0"
...
/* One period at 48 kHz produces a 1 kHz sine wave. */
...
static int audio_test(int argc, char **argv)
{
    ...
    rt_device_t device;
    rt_int16_t *buffer;
    const char *name = AUDIO_TEST_DEVICE;
    struct rt_audio_caps caps = { 0 };

    if (argc > 3)
    {
        rt_kprintf("usage: audio_test [device] [seconds]\n");
        return -RT_EINVAL;
    }
    ...
    if (argc > 2)
    {
        ...
        if (*end || !value || value > AUDIO_TEST_MAX_SECONDS)
        {
    ...
    ret = rt_device_open(device, RT_DEVICE_OFLAG_WRONLY);
    if (ret)
    {
    ...
    ret = rt_device_control(device, AUDIO_CTL_CONFIGURE, &caps);
    if (ret)
    {
    ...
    remaining = (rt_size_t)seconds * AUDIO_TEST_RATE *
            AUDIO_TEST_CHANNELS * sizeof(*buffer);
    rt_kprintf("playing 1 kHz stereo tone on %s for %u second(s)\n",
            name, seconds);
    ...
    while (remaining)
    {
        ...
        /* Interleaved stereo: send the same sample to left and right. */
        for (rt_size_t i = 0; i < samples; i += AUDIO_TEST_CHANNELS)
        {
...
#endif /* RT_USING_AUDIO && RT_AUDIO_ROCKCHIP && RT_USING_FINSH */
