...
#define MSD_CS_LOW()    GPIO_ResetBits(GPIOB, GPIO_Pin_12)
/* Deselect MSD Card: ChipSelect pin high */
#define MSD_CS_HIGH()   GPIO_SetBits(GPIOB, GPIO_Pin_12)
#define MSD_SPI         SPI2
#define MSD_RCC_SPI     RCC_APB1Periph_SPI2
#define SDHC
#ifdef SDHC
u8 flag_SDHC;
#endif
...
u8 MSD_WriteBuffer(u8* pBuffer, u32 WriteAddr, u32 NumByteToWrite)
{
    ...
    while (NbrOfBlock --)
    {
        ...
        if(flag_SDHC == 1)
        {
            ...
            MSD_SendCmd(MSD_WRITE_BLOCK, (WriteAddr + Offset)/512, 0xFF);
        }
        else
#endif //SDHC
            /* Send CMD24 (MSD_WRITE_BLOCK) to write blocks */
...
u8 MSD_ReadBuffer(u8* pBuffer, u32 ReadAddr, u32 NumByteToRead)
{
    ...
    while (NbrOfBlock --)
    {
        ...
        {
            /* Send CMD24 (MSD_WRITE_BLOCK) to write blocks */
            MSD_SendCmd (MSD_READ_SINGLE_BLOCK,(ReadAddr + Offset)/512, 0xFF);
        }
        else
#endif //SDHC
            /* Send CMD17 (MSD_READ_SINGLE_BLOCK) to read one block */
...
u8 MSD_GetCSDRegister(sMSD_CSD* MSD_csd)
{
    ...
    /* MSD chip select low */
    MSD_CS_LOW();
    /* Send CMD9 (CSD register) or CMD10(CID register) */
...
//  MSD_csd->DeviceSize = (CSD_Tab[7] & 0x3F) << 16;
        /* Byte 7 */
        MSD_csd->DeviceSize = (u16)(CSD_Tab[8]) *256;
        /* Byte 8 */
...
u8 MSD_GoIdleState(void)
{
    u8 r1;
    u16 n2,n;
    u8 retvalue = MSD_RESPONSE_FAILURE;
    /* MSD chip select low */
    MSD_CS_LOW();
...
#ifdef SDHC
    MSD_SendCmd(8, 0x1aa, 0x87); /*check version*/
    for (n=0xfff; n>0; n--)
    {
        r1= MSD_ReadByte();
        if(r1 != 0xff)
    ...
    if(r1 == 0x05)
    {
        ...
        /* Send dummy byte 0xFF */
        MSD_WriteByte(DUMMY);
        retvalue = MSD_RESPONSE_NO_ERROR;
#ifdef SDHC
    }
    else if(r1 == 0x01)
    {
...
#endif //sdhc
    if( retvalue == MSD_RESPONSE_NO_ERROR)
    {
//     SPI_Hi_Speed_Config();
    }
    return(retvalue);
...
void MSD_WriteByte(u8 Data)
{
    ...
    while ((MSD_SPI->SR & SPI_I2S_FLAG_TXE)== RESET)
    {
        ;    //(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET);
    }
    /* Send the byte */
    MSD_SPI->DR = Data ; //SPI_I2S_SendData(SPI1, Data);
}
/*******************************************************************************
...
u8 MSD_ReadByte(void)
{
    ...
    while ((MSD_SPI->SR & SPI_I2S_FLAG_RXNE)== RESET)
    {
        ;    //(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET);
    }
    /* Get the received data */
    Data = (u8)(MSD_SPI->DR); //SPI_I2S_ReceiveData(SPI1);
...
void SPI_Config(void)
{
    ...
    RCC_APB1PeriphClockCmd(MSD_RCC_SPI, ENABLE);
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //推挽复用
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;//sck mosi
    GPIO_Init(GPIOB,&GPIO_InitStructure);
