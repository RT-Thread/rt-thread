...
{
    RT_DEBUG_LOG(RT_DEBUG_USB, ("NotifyURBChange_Callback\n"));
...
static int drv_pipe_xfer(upipe_t pipe, rt_uint8_t token, void *buffer, int nbytes, int timeout)
{
    while (1)
    {
        ...
        HAL_HCD_HC_SubmitRequest(&_stm_hhcd_fs,
                                 ...
                                 nbytes,
                                 0);
        rt_completion_wait(&urb_completion, rt_tick_from_millisecond(timeout*10));
...
//    rt_pin_write(USBH_POWER_PIN, PIN_LOW);
	rt_pcf8574_write_bit(USB_PWR_IO, 1);//I2C设备扩展IO
#endif
    return RT_EOK;
}
int stm_usbh_register(void)
{
    ...
    rt_device_register((rt_device_t)uhcd, USB_HOST_CONTROLLER_NAME, 0);
    rt_usb_host_init();
    return RT_EOK;
}
