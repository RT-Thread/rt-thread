...
static void n32_timer_init(struct rt_clock_timer_device *timer, rt_uint32_t state)
{
...
static rt_err_t n32_timer_start(rt_clock_timer_t *timer, rt_uint32_t t, rt_clock_timer_mode_t opmode)
{
...
static void n32_timer_stop(rt_clock_timer_t *timer)
{
    TIM_Module *tim = RT_NULL;
...
static rt_uint32_t n32_timer_counter_get(rt_clock_timer_t *timer)
{
    TIM_Module *tim = RT_NULL;
...
static rt_err_t n32_timer_ctrl(rt_clock_timer_t *timer, rt_uint32_t cmd, void *arg)
{
    RCC_ClocksType RCC_ClockStruct;
    TIM_Module *tim = RT_NULL;
    rt_err_t result = RT_EOK;
...
static int rt_hw_clock_timer_init(void)
{
    ...
    for (i = 0; i < sizeof(n32_clock_timer_obj) / sizeof(n32_clock_timer_obj[0]); i++)
    {
        ...
        if (rt_clock_timer_register(&n32_clock_timer_obj[i].time_device, n32_clock_timer_obj[i].name, n32_clock_timer_obj[i].tim_handle) == RT_EOK)
        {
