...
#define DBG_TAG "clock_timer.k1x"
...
static void spacemit_timer_init(struct rt_clock_timer_device *timer_dev, rt_uint32_t state)
{
    RT_UNUSED(timer_dev);
    RT_UNUSED(state);
}

static rt_err_t spacemit_timer_start(struct rt_clock_timer_device *timer_dev,
        rt_uint32_t cycle, rt_clock_timer_mode_t mode)
{
    rt_ubase_t level;
    struct spacemit_timer_counter *cnt = raw_to_spacemit_timer_counter(timer_dev);
...
static void spacemit_timer_stop(struct rt_clock_timer_device *timer_dev)
{
    rt_ubase_t level;
    struct spacemit_timer_counter *cnt = raw_to_spacemit_timer_counter(timer_dev);
...
static rt_err_t spacemit_timer_ctrl(struct rt_clock_timer_device *timer_dev,
        rt_uint32_t cmd, void *args)
{
    ...
    case CLOCK_TIMER_CTRL_FREQ_SET:
        ...
        if (!err)
        {
            cnt->freq = *(rt_int32_t *)args;
        }
        rt_spin_unlock_irqrestore(&cnt->timer->tm_lock, level);
        break;

    case CLOCK_TIMER_CTRL_STOP:
        spacemit_timer_stop(timer_dev);
        break;
    ...
    case CLOCK_TIMER_CTRL_MODE_SET:
        err = spacemit_timer_start(timer_dev, cnt->cycle,
                (rt_clock_timer_mode_t)(rt_ubase_t)args);
        break;
...
const static struct rt_clock_timer_ops spacemit_timer_ops =
{
...
static rt_err_t spacemit_timer_probe(struct rt_platform_device *pdev)
{
    ...
    rt_ofw_foreach_available_child_node(np, child_np)
    {
        ...
        cnt->parent.freq = cnt->freq;

        cnt->info.maxfreq = cnt->freq;
        cnt->info.minfreq = cnt->freq;
        cnt->info.maxcnt = 0xffffffff;
        cnt->info.cntmode = CLOCK_TIMER_CNTMODE_UP;
        ...
        rt_clock_timer_register(&cnt->parent, dev_name, RT_NULL);
    }
...
static rt_err_t spacemit_timer_remove(struct rt_platform_device *pdev)
{
    ...
    for (int i = 0; i < SPACEMIT_MAX_TIMER; ++i)
    {
        ...
        spacemit_timer_stop(&cnt->parent);
