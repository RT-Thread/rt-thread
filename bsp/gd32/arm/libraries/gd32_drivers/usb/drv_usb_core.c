...
/*!
    ...
    \param[in]  usb_mode: USB host or device mode
    \param[out] none
    \retval     none
*/
void usb_para_init(usb_core_driver *udev, uint32_t usb_periph, uint32_t usb_speed, uint32_t usb_mode)
{
    ...
    usb_basic->core_speed = (uint8_t)usb_speed;
    usb_basic->core_enum = (uint8_t)usb_mode;
    ...
    usb_basic->sof_enable = (usb_basic->core_enum == HOST_MODE) ? 1U : 0U;
...
#else
        if (usb_basic.core_enum == HOST_MODE)
        {
