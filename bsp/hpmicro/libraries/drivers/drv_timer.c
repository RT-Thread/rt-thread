...
static void hpm_clock_timer_init(rt_clock_timer_t *timer, rt_uint32_t state);
static rt_err_t hpm_clock_timer_start(rt_clock_timer_t *timer, rt_uint32_t cnt, rt_clock_timer_mode_t mode);
static void hpm_clock_timer_stop(rt_clock_timer_t *timer);
static rt_uint32_t hpm_clock_timer_count_get(rt_clock_timer_t *timer);
static rt_err_t hpm_clock_timer_control(rt_clock_timer_t *timer, rt_uint32_t cmd, void *args);

static void hpm_hwtmr_isr(hpm_gptimer_t *gptmr);

static const struct rt_clock_timer_ops hpm_clock_timer_ops = {
        .init = hpm_clock_timer_init,
        .start = hpm_clock_timer_start,
        .stop = hpm_clock_timer_stop,
        .count_get = hpm_clock_timer_count_get,
        .control = hpm_clock_timer_control
};

static const struct rt_clock_timer_info hpm_clock_timer_info = {
        .maxfreq = 100000000UL,
        .minfreq = 93750UL,
        .maxcnt = 0xFFFFFFFFUL,
...
static rt_err_t hpm_clock_timer_control(rt_clock_timer_t *timer, rt_uint32_t cmd, void *args)
{
    ...
    case CLOCK_TIMER_CTRL_INFO_GET:
        *(rt_clock_timer_t*)args = hpm_gptmr->timer;
        break;
...
int rt_hw_clock_timer_init(void)
{
    ...
    {
        s_gptimers[i]->timer.info = &hpm_clock_timer_info;
        s_gptimers[i]->timer.ops = &hpm_clock_timer_ops;
