...
static void nu_i2s_isr(int vector, void *param)
{
    ...
    volatile uint32_t u32RegAudCtl = inpw(REG_ACTL_CON);

    if (u32RegAudCtl & I2S_GLBCON_P_DMA_IRQ_Msk)
    {
        volatile uint32_t u32RegPlayStatus = inpw(REG_ACTL_PSR);
        outpw(REG_ACTL_CON, u32RegAudCtl | I2S_GLBCON_P_DMA_IRQ_Msk);  //Clear TX INT

        if (u32RegPlayStatus & I2S_PSP_DMA_CNTER_IRQ_Msk)
        {
            outpw(REG_ACTL_PSR, I2S_PSP_DMA_CNTER_IRQ_Msk);
            rt_kprintf("\ndebug:DMA_COUNTER_IRQ occur\n");
        }

        if (u32RegPlayStatus & I2S_PSP_DMA_DATA_ZERO_IRQ_Msk)
        {
            outpw(REG_ACTL_PSR, I2S_PSP_DMA_DATA_ZERO_IRQ_Msk);
        ...
        if (u32RegPlayStatus & I2S_PSP_P_DMA_RIA_I_Msk)
        {
            ...
            outpw(REG_ACTL_PSR, I2S_PSP_P_DMA_RIA_I_Msk);
    ...
    {
        volatile uint32_t u32RegRecordStatus = inpw(REG_ACTL_RSR);

        outpw(REG_ACTL_CON, u32RegAudCtl | I2S_GLBCON_R_DMA_IRQ_Msk);  //Clear RX INT
        ...
        if (u32RegRecordStatus & I2S_RSR_R_DMA_RIA_IRQ_Msk)
        {
            ...
            */
            ...
            outpw(REG_ACTL_RSR, I2S_RSR_R_DMA_RIA_IRQ_Msk);
