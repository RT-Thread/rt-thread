...
static struct nrf5x_clock_timer nrf5x_clock_timer_obj[] =
{
#ifdef BSP_USING_TIM0
    {
...
{
    rt_clock_timer_t       *timer_device = (struct rt_clock_timer_device *)p_context;

    /* no matter what event_type is(NRF_TIMER_EVENT_COMPARE0 or others), call same function "rt_clock_timer_isr" */
...
static int nrf5x_clock_timer_init(void)
{
    int i = 0;
    int result = RT_EOK;

    for (i = 0; i < sizeof(nrf5x_clock_timer_obj) / sizeof(nrf5x_clock_timer_obj[0]); i++)
    {
        nrf5x_clock_timer_obj[i].timer_device.info = &_info;
        nrf5x_clock_timer_obj[i].timer_device.ops  = &_ops;
        if (rt_clock_timer_register(&nrf5x_clock_timer_obj[i].timer_device, nrf5x_clock_timer_obj[i].name, &nrf5x_clock_timer_obj[i].timer_info) == RT_EOK)
        {
            LOG_D("%s register success", nrf5x_clock_timer_obj[i].name);
        }
        else
        {
            LOG_E("%s register failed", nrf5x_clock_timer_obj[i].name);
            result = -RT_ERROR;
        }
    }
...
INIT_BOARD_EXPORT(nrf5x_clock_timer_init);
