...
{
    return fee_flash_driver_get_status();
}
...
rt_weak fee_ret_t fee_flash_driver_get_caps(fee_flash_caps_t *caps)
{
    if (caps == RT_NULL)
    {
        return FEE_E_PARAM;
...
rt_weak fee_ret_t fee_flash_driver_read(uint32_t addr, uint8_t *dst, uint32_t len)
{
    fee_ret_t ret;
...
rt_weak fee_ret_t fee_flash_driver_write(uint32_t addr, const uint8_t *src, uint32_t len)
{
    fee_ret_t ret;
...
rt_weak fee_ret_t fee_flash_driver_erase(uint32_t addr, uint32_t len)
{
    fee_ret_t ret;
