...
{
    rt_hw_qspi_device_attach("qspi1", "qspi1_0", RT_NULL, 4, RT_NULL, RT_NULL);

    /* init n25qxx */
    if (RT_NULL == rt_sfud_flash_probe("w25q", "qspi1_0"))
    {
        return -RT_ERROR;
    }
...
int can_sample(int argc, char *argv[])
{
    ...
    res = rt_device_control(can_dev, RT_CAN_CMD_SET_BAUD_FD, (void *)CANFD_DATA_4MBaud);
#endif
...
#define EEPROM_I2C_BUS_NAME         "hwi2c1"
#define EEPROM_ADDR                 0x50
rt_uint8_t rx_buff[6] = {0}, tx_buff[6] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc};
...
static rt_err_t read_reg(struct rt_i2c_bus_device *bus, rt_uint8_t reg, rt_uint8_t *data)
{
    ...
    msgs[2].addr = EEPROM_ADDR;
    msgs[2].flags = RT_I2C_WR;
    msgs[2].buf = buf;
    msgs[2].len = 1;

    msgs[3].addr = EEPROM_ADDR;
    msgs[3].flags = RT_I2C_RD;
    msgs[3].buf = data;
    msgs[3].len = 6;
...
MSH_CMD_EXPORT(vcom_sample, vcom test sample);
