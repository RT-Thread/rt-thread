...
static struct ep_id _ep_pool[] =
{
    ...
    {EPADR_HW2SW(EPI),  USB_ENDPOINT_XFER_BULK,        USB_DIR_IN,    EPE_MAX_PKT_SIZE,    ID_UNASSIGNED},
    {EPADR_HW2SW(EPJ),  USB_ENDPOINT_XFER_BULK,        USB_DIR_OUT,   EPF_MAX_PKT_SIZE,    ID_UNASSIGNED},
    {EPADR_HW2SW(EPK),  USB_ENDPOINT_XFER_INT,         USB_DIR_IN,    EPG_MAX_PKT_SIZE,    ID_UNASSIGNED},
    {EPADR_HW2SW(EPL),  USB_ENDPOINT_XFER_INT,         USB_DIR_OUT,   EPH_MAX_PKT_SIZE,    ID_UNASSIGNED},
...
static void _nu_ep_partition_set(int isHighSpeed)
{
    ...
    for (i = 0; i < USBD_MAX_EP; i++)
    {
        ...
        if (_ep_pool[i + 1].type == USB_ENDPOINT_XFER_BULK)
            u32NuEPTypeDef =  USB_EP_CFG_TYPE_BULK;
        else if (_ep_pool[i + 1].type == USB_ENDPOINT_XFER_INT)
...
static void NU_SetupStageCallback(nu_usbd_t *nu_udc)
{
    ...
    setup_packet.bRequestType   = (uint8_t)(nu_udc->base->SETUP1_0 & 0xfful);
