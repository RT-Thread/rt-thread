 ...
 *
 */
#include "gicv2_gicd.h"
#include "gicv2_gicc.h"
#include <rtthread.h>
#include <cp15.h>
#include <bitopr.h>

rt_inline GICD* gicd_get(void)
{
    return (GICD*)(rt_cpu_cbar_get() + RT_GICD_BASE_OFS);
}
...
GIC_VER gicd_get_ver(void)
{
    GICD* gicd = gicd_get();
    // GICD_ICPIDR2
    // [ref]IHI0048B_b_gic_architecture_specification.pdf: 4.3.18 Identification registers
    //      Bit     Field           Description
    //      31-8    reverse         IMPLEMENTATION DEFINED
    //      7-4     ArchRev         0x1/0x2 = GICv1/GICv2
    //      3-0     reverse         IMPLEMENTATION DEFINED
    return (GIC_VER)_E_B_B(gicd->ICPIDR2, 7, 4);
}
...
void gicd_init(void)
{
    RT_ASSERT(gicd_get_ver() == GIC_VER_V1);
    GICD* gicd = gicd_get();
