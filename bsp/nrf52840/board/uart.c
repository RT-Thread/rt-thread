...
static rt_err_t _uart_cfg(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    case 9600:
        config.baudrate = (nrf_uart_baudrate_t)NRF_UART_BAUDRATE_9600;
        break;
    ...
    {
        config.parity = (nrf_uart_parity_t)NRF_UART_PARITY_EXCLUDED;
    }
    else
    {
        config.parity = (nrf_uart_parity_t)NRF_UART_PARITY_INCLUDED;
    }

    config.hwfc = (nrf_uart_hwfc_t)NRF_UART_HWFC_DISABLED;
