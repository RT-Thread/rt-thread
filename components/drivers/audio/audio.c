...
static rt_size_t _audio_dev_write(struct rt_device *dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    ...
    {
        result = rt_data_queue_push(&audio->replay->queue,buffer,size,RT_WAITING_FOREVER);
        if(result != RT_EOK)
        {
...
rt_err_t rt_audio_register(struct rt_audio_device *audio, const char *name, rt_uint32_t flag, void *data)
{
    ...
    //init memory pool for replay
    {
        ...
        rt_mp_init(&audio->mp, "adu_mp", mempool, AUDIO_DEVICE_DECODE_MP_SZ, AUDIO_DEVICE_DECODE_MP_BLOCK_SZ * 2);
    }
