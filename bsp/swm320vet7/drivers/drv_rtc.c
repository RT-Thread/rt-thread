...
static rt_err_t _rtc_close(rt_device_t dev)
{
  return RT_EOK;
}
...
static rt_size_t _rtc_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
...
rt_err_t rt_device_rtc_register(struct rt_device_rtc *device, const char *name, const struct rt_rtc_ops *ops)
{
  rt_err_t result = RT_EOK;
  ...
  device->parent.init = _rtc_init;
  device->parent.open = _rtc_open;
  device->parent.close = _rtc_close;
  device->parent.read = _rtc_read;
  device->parent.write = _rtc_write;
  device->parent.control = _rtc_control;
