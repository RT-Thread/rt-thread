 ...
 *
 */
#ifndef __GIC_GICD_H__
#define __GIC_GICD_H__
...
//      cortex_a9_mpcore_trm_100486_0401_10_en.pdf: 3.3.1 Distributor register summary
typedef struct
{                                                           // offset           type    Description                                                 Cortex-A9
    volatile uint32_t CTLR;                                 // [0x000]          RW      Distributor Control Register.                               supported
    volatile GICD_TYPER TYPER;                              // [0x004]          RO      Interrupt Controller Type Register.                         supported
    volatile uint32_t IIDR;                                 // [0x008]          RO      Distributor Implementer Identification Register.            supported
    ...
    volatile uint32_t IGROUPRn[8];                          // [0x080-0x09C]    RW      Interrupt Group Registers.                                  supported
    volatile uint32_t rev3[(0x100 - 0x0A0) / 4];            // [0x0A0-0x0FC]
    volatile uint32_t ISENABLERn[32];                       // [0x100-0x17C]    RW      Interrupt Set-Enable Registers.                             supported
    volatile uint32_t ICENABLERn[32];                       // [0x180-0x1FC]    RW      Interrupt Clear-Enable Registers.                           supported
    volatile uint32_t ISPENDRn[32];                         // [0x200-0x27C]    RW      Interrupt Set-Pending Registers.                            supported
    volatile uint32_t ICPENDRn[32];                         // [0x280-0x2FC]    RW      Interrupt Clear-Pending Registers.                          supported
    volatile uint32_t ICDABRn[32];                          // [0x300-0x37C]    RW      Active Bit Registers.                                       supported
    volatile uint32_t ICACTIVERn[32];                       // [0x380-0x3FC]    RW      Interrupt Clear-Active Registers
    volatile uint8_t IPRIORITYRn[GIC_INTID_LIMIT];          // [0x400-0x7F8]    RW      Interrupt Priority Registers. (Byte accessible)             supported
    volatile uint32_t rev4;                                 // [0x7FC]
    volatile uint8_t ITARGETSRn[GIC_INTID_LIMIT];           // [0x800-0xBF8]    RW      Interrupt Processor Targets Registers. (Byte accessible)    supported
    volatile uint32_t rev5;                                 // [0xBFC]
    volatile uint32_t ICFGRn[64];                           // [0xC00-0xCFC]    RW      Interrupt Configuration Registers.                          supported
    volatile uint32_t ICSR[64];                             // [0xD00-0xDFC]            Status Register                                             supported
    volatile uint32_t NSACRn[64];                           // [0xE00-0xEFC]    RW      Non-secure Access Control Registers, optional
    volatile uint32_t SGIR;                                 // [0xF00]          WO      Software Generated Interrupt Register                       supported
    ...
    volatile uint32_t ICPIDR[8];                            // [0xFD0-0xFEC]    RO      Peripheral ID0~ID7 register                                 supported
    volatile uint32_t ICCIDR[4];                            // [0xFF0-0xFFC]    RO      Component ID0~ID3 register                                  supported
} GICD;
