...
enum
{
#ifdef BSP_USING_DEVICE_USBOTG1
    USBOTG1_INDEX,
#endif
#ifdef BSP_USING_DEVICE_USBOTG2
    USBOTG2_INDEX,
#endif
};

static struct at32_usbotg usbotgd_config[] = {
#ifdef BSP_USING_DEVICE_USBOTG1
    USBOTG1_CONFIG,
#endif
#ifdef BSP_USING_DEVICE_USBOTG2
...
void OTGFS1_IRQHandler(void)
{
    ...
    usbd_irq_handler(p_usbotg_instance->p_otg_core);
...
{
    usbd_set_stall(&p_usbotg_instance->p_otg_core->dev, address);
    return RT_EOK;
}

static rt_err_t _ep_clear_stall(rt_uint8_t address)
{
    usbd_clear_stall(&p_usbotg_instance->p_otg_core->dev, address);
    return RT_EOK;
}

static rt_err_t _set_address(rt_uint8_t address)
{
    usbd_set_device_addr(&p_usbotg_instance->p_otg_core->dev, address);
    return RT_EOK;
}
...
static rt_err_t _ep_enable(uep_t ep)
{
    ...
    usbd_ept_open(&p_usbotg_instance->p_otg_core->dev, ep->ep_desc->bEndpointAddress, \
...
static rt_err_t _ep_disable(uep_t ep)
{
    ...
    usbd_ept_close(&p_usbotg_instance->p_otg_core->dev, ep->ep_desc->bEndpointAddress);
    return RT_EOK;
}
...
{
    usbd_ept_recv(&p_usbotg_instance->p_otg_core->dev, address, buffer, size);
    return size;
}

static rt_ssize_t _ep_write(rt_uint8_t address, void *buffer, rt_size_t size)
{
    usbd_ept_send(&p_usbotg_instance->p_otg_core->dev, address, buffer, size);
    return size;
}

static rt_err_t _ep0_send_status(void)
{
    usbd_ept_send(&p_usbotg_instance->p_otg_core->dev, 0x00, RT_NULL, 0);
    return RT_EOK;
}
...
static rt_err_t at32_dcd_init(rt_device_t device)
{
    ...
    /* init usb */
    p_usbotg_instance->p_otg_core->usb_reg = usb_global_select_core(p_usbotg_instance->id);

    /* usb device core config */
    usb_core_config(p_usbotg_instance->p_otg_core, p_usbotg_instance->dev_spd);

    if(p_usbotg_instance->p_otg_core->cfg.sof_out)
    {
      p_usbotg_instance->p_otg_core->usb_reg->gccfg_bit.sofouten = TRUE;
    }

    if(p_usbotg_instance->p_otg_core->cfg.vbusig)
    {
      p_usbotg_instance->p_otg_core->usb_reg->gccfg_bit.vbusig = TRUE;
    }

    /* usb device core init */
    usbd_core_init(&(p_usbotg_instance->p_otg_core->dev), p_usbotg_instance->p_otg_core->usb_reg,
...
int at32_usbd_register(void)
{
    ...
    obj_num = sizeof(usbotgd_config) / sizeof(struct at32_usbotg);
    ...
    for (index = 0; index < obj_num; index++) {
        ...
        usbotgd_config[index].p_otg_core = p_otg_core;

        /* register endpoint infomation */
        udcd->ep_pool = endpoint_pool;
        udcd->ep0.id = &endpoint_pool[0];

        result = rt_device_register((rt_device_t)udcd, usbotgd_config[index].name, 0);
        RT_ASSERT(result == RT_EOK);

        p_usbotg_instance = &usbotgd_config[index];
