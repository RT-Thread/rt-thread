...
static IRQn_Type au32GPIRQ[NU_PORT_CNT] = {GPA_IRQn, GPB_IRQn, GPC_IRQn, GPD_IRQn, GPE_IRQn, GPF_IRQn, GPG_IRQn, GPH_IRQn, GPI_IRQn, GPJ_IRQn, GPK_IRQn, GPL_IRQn, GPM_IRQn, GPN_IRQn};
...
static void nu_gpio_isr(int vector, void *param)
{
    ...
    rt_uint32_t port_idx = ((rt_uint32_t)PORT - GPIOA_BASE) / PORT_OFFSET ;
...
int rt_hw_gpio_init(void)
{
    ...
    for (i = 0; i < NU_PORT_CNT ; i++)
    {
        ...
        rt_snprintf(szTmp, sizeof(szTmp), "gpio-%d", i);
        rt_hw_interrupt_install(irqn, nu_gpio_isr, (void *)(GPIOA_BASE + (i * PORT_OFFSET)), szTmp);
    }
