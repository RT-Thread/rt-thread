...
void * gp_renesas_isr_context[BSP_IRQ_VECTOR_MAX_ENTRIES];
...
void bsp_irq_cfg (void)
{
    ...
    {
        volatile uint32_t * ielsr_reg = (&(ICU->ICU_IELSR0_REG) + i);
        *ielsr_reg = (uint32_t) g_interrupt_event_link_select[i];
    }
#endif
}
