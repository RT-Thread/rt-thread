...
int nanosleep(const struct timespec *rqtp, struct timespec *rmtp)
{
    ...
    rt_uint64_t tick = (ns * (1000 * 1000)) / unit;
    ...
    if (rt_get_errno() == -RT_EINTR)
    {
        if (rmtp)
        {
            ...
            rmtp->tv_sec = ((time_t)((rmtp_cpu_tick * unit) / (1000 * 1000))) / NANOSECOND_PER_SECOND;
            rmtp->tv_nsec = ((long)((rmtp_cpu_tick * unit) / (1000 * 1000))) % NANOSECOND_PER_SECOND;
        }
        rt_set_errno(EINTR);
        return -1;
    }
...
int clock_getres(clockid_t clockid, struct timespec *res)
{
    ...
    case CLOCK_CPUTIME_ID:
        res->tv_sec  = 0;
        res->tv_nsec = (clock_cpu_getres() / (1000 * 1000));
        break;
#endif
    ...
    case CLOCK_CPUTIME_ID:
        {
            ...
            tp->tv_sec  = ((uint64_t)((cpu_tick * unit) / (1000 * 1000))) / NANOSECOND_PER_SECOND;
            tp->tv_nsec = ((uint64_t)((cpu_tick * unit) / (1000 * 1000))) % NANOSECOND_PER_SECOND;
        }
        break;
#endif
    default:
...
static void rtthread_timer_wrapper(void *timerobj)
{
    ...
    {
        timer->reload = ((timer->interval.tv_sec * NANOSECOND_PER_SECOND + timer->interval.tv_nsec) * (1000 * 1000)) / clock_cpu_getres();
        if (timer->reload)
...
int timer_gettime(timer_t timerid, struct itimerspec *its)
{
    ...
    if (timer->status == ACTIVE)
    {
        ...
        if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout())
        {
            ...
            remaining = ((remain_tick - clock_cpu_gettime()) * (1000 * 1000)) / clock_cpu_getres();
                  ...
                  struct itimerspec *ovalue)
{
    ...
    if (timer->clockid == CLOCK_CPUTIME_ID && clock_cpu_issettimeout())
    {
        ...
        tick = ((value->it_value.tv_sec * NANOSECOND_PER_SECOND + value->it_value.tv_nsec) * (1000 * 1000)) / unit;
        if ((flags & TIMER_ABSTIME) == TIMER_ABSTIME)
        {
