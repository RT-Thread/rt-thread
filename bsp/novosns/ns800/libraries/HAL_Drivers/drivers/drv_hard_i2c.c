...
/**
 * @brief   Wait for master TX FIFO to have space available with timeout
 * @param   i2c I2C instance
 * @param   timeout_ms timeout in milliseconds
 * @return  I2C_STATUS_SUCCESS if ready, otherwise error status
 */
static I2C_Status I2C_waitMasterTxReadyTimeout(I2C_TypeDef *i2c, uint32_t timeout_ms)
{
    ...
    while (rt_tick_get() - start_tick < rt_tick_from_millisecond(timeout_ms))
    {
        ...
        if (txCount > 0U)
        {
            ...
            return I2C_checkAndClearMasterError(i2c, status);
        }
...
/**
 * @brief   Send STOP signal with timeout protection
 * @param   i2c I2C instance
 * @param   timeout_ms timeout in milliseconds
 * @return  I2C_STATUS_SUCCESS if success, otherwise error status
 */
static I2C_Status I2C_sendStopTimeout(I2C_TypeDef *i2c, uint32_t timeout_ms)
{
    uint32_t start_tick = rt_tick_get();
    uint32_t status;

    LOG_D("I2C_sendStopTimeout: entering, timeout=%dms", timeout_ms);

    /* Wait until there is room in the fifo. */
    I2C_Status result = I2C_waitMasterTxReadyTimeout(i2c, timeout_ms);
    if (result != I2C_STATUS_SUCCESS)
    {
        LOG_E("I2C_sendStopTimeout: TxReady failed, result=%d", result);
        return result;
    }

    LOG_D("I2C_sendStopTimeout: TxReady OK, sending STOP");
 ...
 * @param   i2c I2C instance
 * @return  I2C_STATUS_SUCCESS if idle, otherwise I2C_STATUS_BUSY
 */
...
/**
 * @brief   Reset I2C bus to recover from error state
 * @param   i2c I2C instance
 */
static void I2C_resetBus(I2C_TypeDef *i2c)
{
    /* Reset master module */
    I2C_resetMaster(i2c);
...
static void ns800_i2c_gpio_init(struct ns800_i2c_config *cfg)
{
    ...
    LOG_D("I2C[%s] GPIO init: SCL(port=0x%x, pin=%d, alt=%d), SDA(port=0x%x, pin=%d, alt=%d)",
          cfg->name, cfg->scl_port, cfg->scl_pin, cfg->scl_alt_func,
          cfg->sda_port, cfg->sda_pin, cfg->sda_alt_func);
...
static rt_err_t ns800_i2c_init(struct ns800_i2c *i2c_drv)
{
    ...
    LOG_D("I2C[%s] init start, Instance=0x%x, baudrate=%d", 
          cfg->name, i2c, cfg->baudrate);
                                         ...
                                         rt_uint32_t num)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & RT_I2C_NO_STOP))
        {
            ...
            status = I2C_sendStopTimeout(i2c, timeout);
            if (status != I2C_STATUS_SUCCESS)
            {
...
out:
    ...
    I2C_sendStopTimeout(i2c, timeout / 2);  /* Shorter timeout for recovery */
    return ret;
}
