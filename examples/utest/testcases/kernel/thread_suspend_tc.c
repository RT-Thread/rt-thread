 ...
 * Change Logs:
 * Date           Author       Notes
 * 2025-09-02     Rbb666       utest case for rt_thread_suspend_force comprehensive tests
 */
...
static void monitor_control_thread(void *parameter)
{
    ...
    /* Use rt_thread_suspend_force to suspend target thread */
    if (rt_thread_suspend_force(target_thread, RT_UNINTERRUPTIBLE) == RT_EOK)
    {
...
/* Test normal usage of rt_thread_suspend_force function */
...
static void test_suspend_force_api_basic(void)
{
    ...
    rt_err_t result = rt_thread_suspend_force(api_thread, RT_UNINTERRUPTIBLE);
...
static void test_suspend_force_deadlock_risk(void)
{
    ...
    rt_err_t suspend_result = rt_thread_suspend_force(holder_thread, RT_UNINTERRUPTIBLE);
    ...
    /* Verify rt_thread_suspend_force and rt_thread_resume executed successfully */
    uassert_true(suspend_result == RT_EOK);
    uassert_true(resume_result == RT_EOK);
