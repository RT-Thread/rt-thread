...
static rt_bool_t riscv_intc_use_aia(void)
{
#if defined(RT_PIC_RISCV_AIA) && !defined(PIC_DISABLE_DM)
    return riscv_isa_ext_test(SxAIA) && imsic_ofw_is_available();
#else
    return RT_FALSE;
#endif
}
...
rt_inline void riscv_intc_irq_mask_raw(struct riscv_intc *rv_intc, int hwirq)
{
    ...
    if (hwirq >= rv_intc->ipi_base)
    {
        ...
        rt_hw_atomic_and(ipi_enable, ~BIT(hwirq - rv_intc->ipi_base));
...
rt_inline void riscv_intc_irq_unmask_raw(struct riscv_intc *rv_intc, int hwirq)
{
    ...
    if (hwirq >= rv_intc->ipi_base)
    {
        ...
        rt_hw_atomic_or(ipi_enable, BIT(hwirq - rv_intc->ipi_base));
        hwirq = RV_IRQ_SOFT;
    }
#endif /* RT_USING_SMP */
...
rt_inline void riscv_intc_irq_send_ipi_raw(struct riscv_intc *rv_intc, int ipi, rt_bitmap_t *cpumask)
{
...
#ifdef RT_USING_SMP
    RT_ASSERT(ipi >= rv_intc->ipi_base && ipi < rv_intc->ipi_base + RT_MAX_IPI);
    ipi -= rv_intc->ipi_base;
#endif
    ...
    #ifdef RT_USING_SMP
        rt_hw_atomic_or(&rv_intc->ipi_pending[cpu], BIT(ipi));
    #endif
        ...
        rt_bool_t (*callback)(int hwirq, void *data), void *data)
{
    ...
    else
    {
        ...
        pending = rt_hw_atomic_exchange(&rv_intc->ipi_pending[rt_hw_cpu_id()], 0);

        rt_bitmap_for_each_set_bit(&pending, ipi, RT_MAX_IPI)
        {
            res = callback(rv_intc->ipi_base + ipi, data) || res;
        }
        ...
        rt_bool_t (*callback)(int hwirq, void *data), void *data)
{
    ...
    {
        res = riscv_intc_common_handler_raw(rv_intc, topi, callback, data) || res;
    }
