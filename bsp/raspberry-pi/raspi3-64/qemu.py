...
dts_name = "bcm283x-rpi-3-b"

dtc.dts_to_dtb(RTT_ROOT, ["../dm/dts/" + dts_name + ".dts"], include_paths = [os.getcwd() + '/../dm/include'],
		ignore_warning = ["simple_bus_reg", "unit_address_vs_reg", "clocks_is_cell", "gpios_property", "unique_unit_address"])
try:
...
for i in range(len(opt)):
	...
	if is_opt("graphic", inkey): q_graphic = "-device usb-kbd,port=1.2 -device usb-mouse,port=1.3"
...
{}qemu-system-aarch64 \
	...
	-kernel kernel8.img \
	-dtb {}.dtb \
	-append "console=ttyAMA0 earlycon=pl011,0x3f201000 cma=8M coherent_pool=2M root=sd0p0 rootfstype=elm rootwait rw" \
	-serial mon:stdio \
	...
	-device usb-hub,port=1 \
		-device usb-net,netdev=net0,port=1.1 -netdev user,id=net0 \
	{} \
	{}
