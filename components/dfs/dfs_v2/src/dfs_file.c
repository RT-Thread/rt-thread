...
void dfs_file_unref(struct dfs_file *file)
{
    ...
    if (ret == RT_EOK)
    {
        ...
        if (rt_atomic_load(&(file->ref_count)) == 0)
        {
    ...
    if (*mnt && fullpath)
    {
        ...
        while (*fp != '\0')
        {
            ...
            if (*fp == '/')
            {
                ...
                {
                    int ret = dfs_file_readlink(path, link_fn, DFS_PATH_MAX);
                    if (ret > 0)
                    {
        ...
        if (fullpath)
        {
            ...
            if (mnt)
            {
                ...
                if (dentry)
                {
                    ...
                    {
                        DLOG(msg, "dfs_file", mnt->fs_ops->name, DLOG_MSG, "no fops->open");
                    }
                }
                else
                {
...
ssize_t dfs_file_read(struct dfs_file *file, void *buf, size_t len)
{
    ...
    if (file)
    {
        ...
        else if (file->dentry->vnode->type != FT_DIRECTORY)
        {
            off_t pos;
            pos = file->fpos;
...
int dfs_file_link(const char *oldname, const char *newname)
{
    ...
    if (dfs_file_stat(newname, &stat) >= 0)
    {
        return ret;
    }
    ...
    if (old_fullpath)
    {
        ...
        char *tmp = dfs_nolink_path(&mnt, old_fullpath, 1);
        if (tmp)
        {
            rt_free(old_fullpath);
            old_fullpath = tmp;
    ...
    if (new_fullpath)
    {
        char *tmp = dfs_nolink_path(&mnt, new_fullpath, 1);
        if (tmp)
        {
    ...
    if (target && linkpath)
    {
        ...
        if (dfs_file_access(fullpath, O_RDONLY) != 0)
        {
            ...
            if (parent)
            {
                ...
                if (mnt)
                {
                    char *tmp = dfs_nolink_path(&mnt, parent, 1);
                    if (tmp)
                    {
                        rt_free(parent);
                        parent = tmp;
        ...
        if (link_fn)
        {
            ...
            do
            {
                ...
                if (length > 0)
                {
                    ...
                    DLOG(msg, "dfs", "dfs_file", DLOG_MSG, "dfs_file_stat(%s, &stat)", fullpath);
