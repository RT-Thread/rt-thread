...
rt_err_t spacemit_firmware_load_elf(const char *name, rt_ubase_t phy_addr)
{
    ...
    for (int i = 0; i < ehdr->e_phnum; ++i)
    {
        ...
        /* Sparse remoteproc metadata (for example .resource_table) has its
         * own device address and reserved-memory mapping.  Relocating it with
         * the executable image would write to an unrelated DDR address. */
        if (p->p_paddr > ehdr->e_entry + MAX_IOMAP_SIZE)
        {
            continue;
        }
        ...
        while (memsz > 0)
        {
            ...
            if (!(vma = rt_ioremap_nocache((void *)seg_phys, chunk)))
            {
                return -RT_ENOMEM;
            }
