...
int dfs_wffs_mount(struct dfs_filesystem *fs, unsigned long rwflag, const void *data)
{
...
int dfs_wffs_unmount(struct dfs_filesystem *fs)
{
	return DFS_STATUS_OK;
}

int dfs_wffs_ioctl(struct dfs_fd *file, int cmd, void *args)
{
	return -DFS_STATUS_EIO;
}

struct fsdata_file *dfs_wffs_lookup(struct fsdata_file *root_dirent, const char *path, rt_size_t *size)
{
...
int dfs_wffs_read(struct dfs_fd *file, void *buf, rt_size_t count)
{
	rt_size_t length;
...
int dfs_wffs_lseek(struct dfs_fd *file, rt_off_t owffset)
{
	if (owffset <= file->size)
	{
		file->pos = owffset;
		return file->pos;
	}
...
int dfs_wffs_close(struct dfs_fd *file)
{
	file->data = RT_NULL;
	return DFS_STATUS_OK;
}

int dfs_wffs_open(struct dfs_fd *file)
{
	rt_size_t size;
	struct fsdata_file *dirent;
	struct fsdata_file *root_dirent;
	...
	dirent = dfs_wffs_lookup(root_dirent, file->path, &size);
	if (dirent == RT_NULL)
...
int dfs_wffs_stat(struct dfs_filesystem *fs, const char *path, struct stat *st)
{
	rt_size_t size;
	struct fsdata_file *dirent;
	struct fsdata_file *root_dirent;
	...
	dirent = dfs_wffs_lookup(root_dirent, path, &size);
...
int dfs_wffs_getdents(struct dfs_fd *file, struct dirent *dirp, rt_uint32_t count)
{
	rt_size_t index;
	const char *name;
	struct dirent *d;
	...
	dirent = (struct fsdata_file *)file->data;
	RT_ASSERT(dirent->type == wffs_DIRENT_DIR);
...
static const struct dfs_filesystem_operation _wffs = 
{
	"wffs",
	DFS_FS_FLAG_DEFAULT,
	dfs_wffs_mount,
