        ...
        struct rt_device *slave, void *fw_data)
{
    ...
    rt_bool_t matched_reserved = RT_FALSE;
    ...
    if (drcmr >= 0)
    {
        ...
        {
            int chan_id = pdma->reserved_channels[i].chan_id;

            if (pdma->reserved_channels[i].drcmr == drcmr &&
                chan_id >= 0 && chan_id < pdma->dma_channels)
            {
                matched_reserved = RT_TRUE;

                if (!pdma->chan[chan_id].desc)
                {
                    chan = &pdma->chan[chan_id];
                }

                goto _found_reserved;
            }
        }
    }

_found_reserved:
    if (matched_reserved && !chan)
    {
    ...
    for (int i = 0; i < pdma->dma_channels; ++i)
    {
        ...
        if (!pdma->chan[i].desc)
        {
            ...
            {
                if (pdma->reserved_channels[j].chan_id == i)
                {
                    reserved = RT_TRUE;
                    break;
                }
