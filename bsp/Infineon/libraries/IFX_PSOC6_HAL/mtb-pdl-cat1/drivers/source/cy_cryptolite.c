...
{
    if((REG_CRYPTOLITE_STATUS(base) & CRYPTO_STATUS_BUSY_Msk) != 0UL)
    {
    ...
    while((REG_CRYPTOLITE_STATUS(base) & CRYPTO_STATUS_BUSY_Msk) != 0UL){};
    if((REG_CRYPTOLITE_SHA_INTR_ERROR(base) & CRYPTO_INTR_ERROR_BUS_ERROR_Msk) != 0UL)
    {
...
*
*******************************************************************************/
static cy_en_cryptolite_status_t Cy_Cryptolite_Sha256_Process_aligned(CRYPTO_Type *base,
                                        cy_stc_cryptolite_context_sha_t *cfContext,
