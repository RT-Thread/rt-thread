...
struct rtgui_image_bmp
{
	...
	rt_uint32_t Rmask; //红色分量
	rt_uint32_t Gmask; //绿色分量
	rt_uint32_t Bmask; //蓝色分量
...
struct rtgui_image_bmp_header
{
	...
	char   magic[2];	        //BMP文件标识"BM"
	rt_uint32_t bfSize;			//文件大小
	rt_uint16_t bfReserved1;	//保留字1
	rt_uint16_t bfReserved2;	//保留字2
	rt_uint32_t bfOffBits;		//偏移量

	/* The Win32 BITMAPINFOHEADER struct (40 bytes) */
	rt_uint32_t biSize;			//信息头大小
	rt_int32_t  biWidth;		//位图宽
	rt_int32_t  biHeight;		//位图高
	rt_uint16_t biPlanes;		//图层数
	rt_uint16_t biBitCount;		//颜色深度
	rt_uint32_t biCompression;	//是否压缩
	rt_uint32_t biSizeImage;	//图像大小
	rt_int32_t  biXPelsPerMeter;//水平方向每米像素数
	rt_int32_t  biYPelsPerMeter;//垂直方向每米像素数
	rt_uint32_t biClrUsed;
	rt_uint32_t biClrImportant;
};

static rt_bool_t rtgui_image_bmp_check(struct rtgui_filerw* file);
static rt_bool_t rtgui_image_bmp_load(rtgui_image_t* image, struct rtgui_filerw* file, rt_bool_t load);
static void rtgui_image_bmp_unload(rtgui_image_t* image);
static void rtgui_image_bmp_blit(rtgui_image_t* image, rtgui_dc_t *dc, rtgui_rect_t* rect);

struct rtgui_image_engine image_bmp_engine =
{
	"bmp",
	{ RT_NULL },
...
static rt_bool_t rtgui_image_bmp_load(rtgui_image_t* image, struct rtgui_filerw* src, rt_bool_t load)
{
	rt_uint32_t Rmask;
	rt_uint32_t Gmask;
	rt_uint32_t Bmask;
	...
	header = (struct rtgui_image_bmp_header*) rt_malloc(sizeof(struct rtgui_image_bmp_header));
	if(header == RT_NULL) return RT_FALSE;

	if(rtgui_filerw_read(src, header->magic, 1, 2) == 2)
	{	
	...
	switch (header->biCompression)
	{
		case BI_RGB:
			...
			/* Fall through -- read the RTGUI_RGB masks */
	...
	bmp = (struct rtgui_image_bmp*) rt_malloc(sizeof(struct rtgui_image_bmp));
	if (bmp == RT_NULL)
		goto __exit;
	...
	if (load == RT_TRUE)
	{
		...
		/* load all pixels */
		bmp->pixels = rt_malloc(image->h * bmp->pitch);
	...
	{
		bmp->pixels = RT_NULL;
		bmp->line_pixels = rt_malloc(bmp->pitch);
	}
...
static void rtgui_image_bmp_unload(rtgui_image_t* image)
{
	...
	if (image != RT_NULL)
	{
		...
		if (bmp->pixels != RT_NULL) rt_free(bmp->pixels);
		if (bmp->line_pixels != RT_NULL) rt_free(bmp->line_pixels);
...
static void rtgui_image_bmp_blit(rtgui_image_t* image, rtgui_dc_t *dc, rtgui_rect_t* dst_rect)
{
	int y;
	rt_uint16_t w, h;
	struct rtgui_image_bmp* bmp;
	...
	if(bmp->pixels != RT_NULL)
	{
		...
		else
		{
			...
			if(image->palette == RT_NULL)
			{
				...
				line_ptr = (rt_uint8_t*) rt_malloc(hw_driver->byte_per_pixel * w);
				...
				if(line_ptr != RT_NULL)
				{
					for(y = 0; y < h; y ++)
					{
						...
						dc->engine->blit_line(dc, dst_rect->x1, dst_rect->x1 + w, dst_rect->y1 + y, line_ptr);
						ptr += bmp->pitch;
					}
				}
	...
	else
	{
		...
		{
			rt_uint8_t *line_ptr = (rt_uint8_t*) rt_malloc(hw_driver->byte_per_pixel * w);
...
void rtgui_image_bmp_init()
{
	...
	rtgui_image_register_engine(&image_bmp_engine);
}
