...
static void spacemit_mailbox_release(struct rt_mbox_chan *chan)
{
    ...
    HWREG32(sp_mbox->regs + MBOX_REG_ICR) =
            RT_BIT(chan - sp_mbox->parent.chans);
}
...
static rt_err_t spacemit_mailbox_send(struct rt_mbox_chan *chan, const void *data)
{
    ...
    HWREG32(sp_mbox->regs + MBOX_REG_ISRW) =
            RT_BIT(chan - sp_mbox->parent.chans);
...
static rt_bool_t spacemit_mailbox_peek(struct rt_mbox_chan *chan)
{
    ...
    return !!(HWREG32(sp_mbox->regs + MBOX_REG_RDR) &
            RT_BIT(chan - sp_mbox->parent.chans));
}
...
static void spacemit_mailbox_isr(int irqno, void *param)
{
    ...
    {
        chan = &sp_mbox->parent.chans[i - SPACEMIT_TX_ACK_OFFSET];
...
static void spacemit_mailbox_free_resource(struct spacemit_mailbox *sp_mbox)
{
    ...
    if (!rt_is_err_or_null(sp_mbox->clk))
    {
        if (sp_mbox->clk_enabled)
        {
            rt_clk_disable_unprepare(sp_mbox->clk);
        }
        rt_clk_put(sp_mbox->clk);
    }
...
static rt_err_t spacemit_mailbox_probe(struct rt_platform_device *pdev)
{
    ...
    sp_mbox->clk = rt_clk_get_by_name(dev, "core");

    if (rt_is_err(sp_mbox->clk))
    {
        err = rt_ptr_err(sp_mbox->clk);
        goto _fail;
    }

    if ((err = rt_clk_prepare_enable(sp_mbox->clk)))
    {
        goto _fail;
    }
    sp_mbox->clk_enabled = RT_TRUE;
