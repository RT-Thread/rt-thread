...
{
    BASE_CLOCK             = 0x0,
    BASE_CLOCK_DIV_BY_2    = 0x1,
    BASE_CLOCK_DIV_BY_4    = 0x2,
    BASE_CLOCK_DIV_BY_8    = 0x4,
    BASE_CLOCK_DIV_BY_16   = 0x8,
    BASE_CLOCK_DIV_BY_32   = 0x10,
    BASE_CLOCK_DIV_BY_64   = 0x20,
    BASE_CLOCK_DIV_BY_128  = 0x40,
    BASE_CLOCK_DIV_BY_256  = 0x80,
}sdioCardClkDiv;
...
static int g_sdioError[SDIO_HANDLE_COUNT]      = {SDIO_OK, SDIO_OK};
static int g_cmdStatus[SDIO_HANDLE_COUNT]      = {SDIO_WAIT, SDIO_WAIT};
static int g_transferStatus[SDIO_HANDLE_COUNT] = {SDIO_WAIT, SDIO_WAIT};
...
#define SD_APP_SET_WR_ERASE_COUNT  23   /* ac   [22:0] number of blocks  R1  */
...
static void sdioSetBlkSizeRegister(U32 index, U8 boundary, unsigned short blksize)
{
    GH_SDIO_set_BlkSizeReg_HostSdmaBufSize(index, boundary);
    GH_SDIO_set_BlkSizeReg_TraBlkSize(index, blksize);
}

static void sdioSetBlkCntRegister(U32 index, unsigned  short  blkcnt)
{
...
static void GD_SDIO_Rest(U32 index)
{
    sdioSoftReset(index);
    sdioClockOnOff(index,0);
    sdioSetClockDiv(index,0x40);
    ...
    g_sdioError[index]      = SDIO_OK;
    g_cmdStatus[index]      = SDIO_WAIT;
    g_transferStatus[index] = SDIO_WAIT;
...
static int  sdioIssueCmd(U32 index,U32 cmd, U32 arg, U32 data, U32 flags)
{
    ...
    while(g_cmdStatus[index] !=SDIO_ERR_CMD && g_cmdStatus[index] != SDIO_CMD_COMPLETE)
     {
    ...
    g_cmdStatus[index] = SDIO_WAIT;
    if(g_sdioError[index] != GD_OK)
    {
        g_sdioError[index] = GD_OK;
        return -1;
    }
    return GD_OK;
}
...
static int GD_SDIO_GetCsd(sdioBlockT *cardInfo)
{
    ...
    /*CSD structure version (0: CSD version 1.0-SDSC; 1: CSD version 2.0-SDHC/SDXC)*/
    ...
    cardInfo->tran_speed = freq * mult;                /*speed:25000000MHz*/
    /*r/w block length*/
    cardInfo->read_bl_len = 1 << ((resp[cardInfo->index][1] >> 16) & 0xf);
    readBlkLen = cardInfo->read_bl_len;
    ...
    if (cardInfo->csdInfo.csdStructure)
    {
        csize = (resp[cardInfo->index][1] & 0x3f) << 16 | (resp[cardInfo->index][2] & 0xffff0000) >> 16;
        cmult = 8;
        cardInfo->capacity_user = (csize + 1) * 512 * 1024;
    }
    else
    {
        csize = (resp[cardInfo->index][1] & 0x3ff) << 2 | (resp[cardInfo->index][2] & 0xc0000000) >> 30;
        cmult = (resp[cardInfo->index][2] & 0x00038000) >> 15;
        cardInfo->capacity_user = ((csize + 1) << (cmult + 2)) * readBlkLen;
    }
    /* sectorcount*/
    cardInfo->sectorcount=((csize + 1) << (cmult + 2)) * temp;

    userCap = cardInfo->capacity_user >> 30;
    if (userCap == 0)
    {
        userCap = (cardInfo->capacity_user * 100) >> 20;    //Uint: MByte
        unit[0] = 'M';
    }
    else
    {
        userCap = (cardInfo->capacity_user * 100) >> 30;    //Uint: GByte
        unit[0] = 'G';
    }    
    userCapInte = (U32)(userCap / 100);    //Integer part
    userCapDec = (U32)(userCap % 100);     //Decimal part
#ifdef DEBUG_PRINT
        GM_Printf("Card user capacity: %d.%d%s\n", userCapInte, userCapDec, unit);
#endif
    /*Get devicesize*/
...
{
    int ret=0;
    ret = sdioIssueCmd(cardInfo->index, SD_SEND_RELATIVE_ADDR, 0x00000000,0, MMC_RSP_R6);
    if(ret)
    {
        return -1;
    }
...
static int GD_SDIO_GetScr(sdioBlockT *cardInfo)
{
    ...
    {
        sdioSetBlkSizeRegister(cardInfo->index, SD_DEFAULT_BOUNDARY_ARG, 8);
        ...
        while(g_transferStatus[index] !=SDIO_ERR_CMD && g_transferStatus[index] != SDIO_TRANSFER_COMPLETE)
        {
    ...
    switch (cardInfo->scr.sdSpec)
    {
        case 0:
            version = 10;
            if (cardInfo->type != CARDTYPE_SD10)
            {
                cardInfo->type = CARDTYPE_SD10;
            }
#ifdef DEBUG_PRINT
            printf("This is SD Ver1.0 card\n");
#endif
        break;
        case 1:
            version = 11;
            if (cardInfo->type != CARDTYPE_SD11)
            {
                cardInfo->type = CARDTYPE_SD11;
            }
#ifdef DEBUG_PRINT
            printf("This is SD Ver1.1 card\n");
#endif
        break;
        case 2:
            version = 20;
            if (cardInfo->type != CARDTYPE_SD20)
            {
                cardInfo->type = CARDTYPE_SD20;
            }
#ifdef DEBUG_PRINT
            printf("This is SD Ver2.0 card\n");
#endif
        break;
        default:
#ifdef DEBUG_PRINT
            printf("Unrecognized SD spec ver card\n");
#endif
        break;
   }
...
********************************************************************************
*/
static int GD_SDIO_SDSC_Init(sdioBlockT *cardInfo)
{
    int ret=0;
    U32 hcs=0;
    int retries=1000;
    ...
    /* Supply Voltage: 2.7~3.6V   Check Pattern: 0xaa */
    ...
    while (retries--)
    {
        ...
        if ((resp[cardInfo->index][0] >> 24) & (1 << 7))//bit31 : busy states = 1 (ready)
        {
            if((resp[cardInfo->index][0] >> 24) & (1<<6))//bit30: CCS = 1 (SDHC/SDXC)
            {
                cardInfo->capacity = CARDCAPACITY_SDHC_OR_SDXC;
                cardInfo->highCapacity = 1;
#ifdef DEBUG_PRINT
                printf("This is SDHC or SDXC card\n");
#endif
            }
            else//bit30: CCS = 0 (SDSC)
            {
                cardInfo->capacity = CARDCAPACITY_SDSC;
                cardInfo->highCapacity = 0;
#ifdef DEBUG_PRINT
                printf("This is SDSC card\n");
#endif
            }
            retries = -2;
            break;
...
********************************************************************************
*/
static GERR GD_SDIO_SDHC_SDXC_Init(sdioBlockT *cardInfo)
{
    int ret=0;
    U32 hcs=0;
    int retries=1000;
...
static GERR GD_SDIO_CardInit(U32 index)
{
    ...
    ret = GD_SDIO_SDHC_SDXC_Init(cardInfo);
    if(ret == -1)
    {
        if(GD_SDIO_SDSC_Init(cardInfo) == -1)
        {
#ifdef DEBUG_PRINT
            printf("no card info\n");
#endif
    ...
    highSpeedEnable = SWITCH_HIGH_SPEED;
    if (highSpeedEnable)
    {
...
#if SDIO_DATA_BUS_WIDTH == SWITCH_1BIT_MODE
    busWidth = 1;
#else
    busWidth = 4;
#endif
...
{
    U8 cardInserted = 0;
    U8 cardDetPinLevel = 0;
    U8 cardStateStable = 0;

    cardInserted    = GH_SDIO_get_PresentStateReg_CardInserted(index);
    cardDetPinLevel = GH_SDIO_get_PresentStateReg_CardDetectPinLevel(index);
    cardStateStable = GH_SDIO_get_PresentStateReg_CardStateStable(index);
    
    if(GD_SDIO_GetSdState(index)==SDIO_COM_STATE_CARD_DETECTION)
    {
        if(cardInserted && cardDetPinLevel && cardStateStable)
        {
...
static int GD_SDIO_WriteBlocks(sdioBlockT *cardInfo, U32 isusedma,void *scr, U32  start,U32  blkcnt)
{
    ...
    if(cmdidx==MMC_WRITE_MULTIPLE_BLOCK)
    {
        ...
        {
            if(sdioIssueCmd(cardInfo->index,SD_APP_SET_WR_ERASE_COUNT,blkcnt,0,MMC_RSP_R1))
                return -1;
        }
    }
#endif
    ...
    {
        while(g_transferStatus[index] !=SDIO_ERR_CMD &&
              g_transferStatus[index] != SDIO_TRANSFER_COMPLETE)
        {	
...
{
    U32 index = 0;
    U32 irq_status_reg=(GH_SDIO_get_NorIntStaReg(index) | \
    ...
    if( irq_status_reg & SDIO_ERROR_IRQ )
    {
        ...
        GH_SDIO_set_SoftResetReg_SoftwareResetDatLine(index,1);
        GH_SDIO_set_SoftResetReg_SoftwareResetCmdLine(index,1);
        g_cmdStatus[index] = SDIO_ERR_CMD;
        g_transferStatus[index] = SDIO_ERR_CMD;
        g_sdioError[index] = (irq_status_reg>>16);
#ifdef DEBUG_PRINT
        printf(" - Error IRQ %x\n", g_sdioError);
#endif
    ...
    else if( irq_status_reg & SDIO_IRQ_CMD_COMPLETE )
    {
        ...
        g_sdioError[index] = 0;
        g_cmdStatus[index] = SDIO_CMD_COMPLETE;
        g_transferStatus[index] = SDIO_WAIT;
    ...
    else if( irq_status_reg & SDIO_IRQ_TRANSFER_COMPLETE )
    {
        ...
        ts=0;
        //printf("trans complete irq_status_reg:0x%08x\n",irq_status_reg);
    }
}
...
GERR GD_SDIO_Init(GD_SDIO_OpenParamsT* openParamsP, U32 index)
{
    ...
    ret = GD_INT_Open(&intParams0, psdioHandle0);
    if(ret != GD_OK)
    {
        return GD_ERR_SDIO_INT_ERR;
    }
    //GD_INT_SetHandler(intParams0.type, GD_SDIO_ISR);
    GD_INT_Enable(psdioHandle0,GD_INT_ENABLED);
    /*open the sdio detec interrupt*/
    intParams1.type           = detect_irq[index];
    ...
    ret = GD_INT_Open(&intParams1, psdioHandle1);
    if(ret != GD_OK)
    {
        return GD_ERR_SDIO_INT_ERR;
    }
	//GD_INT_Enable(psdioHandle1,GD_INT_ENABLED);
