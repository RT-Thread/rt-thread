/*
 * File Name	 main.cpp
 * Descriptions	 main function entrance
 * Change Logs
 * Date             Author       Notes
...
{
    LOG_D("set_com_mode() mode=%d\r\n", mode);

    if ((COM_MODE_BT != mode)
            && (COM_MODE_VCOM != mode))
    {
        // 模式无效
        LOG_D("set_com_mode() failed, invalid mode(%d)!\r\n", mode);
        return false;
    }
...
int main(void)
{
    //ulog_tag_lvl_filter_set("[main.cpp]", LOG_LVL_WARNING);
    int main_ret = 0;
    rt_err_t ret = RT_EOK;
    //rt_kprintf("BUILD=%s\r\n",BUILD);
    LOG_I("LOG_I: BUILD=%s", BUILD);
    /* 创建Vcom发送循环队列 */
    ...
    if (RT_NULL == vcom_dev)
    {
        LOG_D("main() call rt_device_find(%s) failed!\r\n", VCOM_DEVICE_NAME);
        //main_ret = -RT_ERROR;
        //goto _END;
    }
    else
    {
        ...
        if (RT_EOK != ret)
        {
            LOG_D("main() call rt_device_open(vcom_dev) failed, error(%d)!\r\n", ret);
            //main_ret = ret;
            //goto _END;
        }
        else
        {
            ...
            if (RT_EOK != ret)
            {
                LOG_D("main() call rt_device_set_rx_indicate(vcom_dev) failed, error(%d)!\r\n", ret);
                //main_ret = ret;
                //goto _END;
            }
            ...
            if (RT_EOK != ret)
            {
                LOG_D("main() call rt_device_set_tx_complete(vcom_dev) failed, error(%d)!\r\n", ret);
                //main_ret = ret;
                //goto _END;
            }
        }
