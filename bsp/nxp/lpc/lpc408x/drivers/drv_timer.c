...
static rt_err_t timer_start(rt_clock_timer_t *timer, rt_uint32_t t, rt_clock_timer_mode_t opmode)
{
    ...
    match.StopOnMatch = (opmode == CLOCK_TIMER_MODE_ONESHOT) ? ENABLE : DISABLE;
...
static rt_clock_timer_t _timer0;
...
int rt_hw_clock_timer_init(void)
{
    ...
    rt_clock_timer_register(&_timer0, "timer0", LPC_TIM0);
