...
static size_t _unmap_area_one(rt_ubase_t *top_table, void *v_addr)
{
    ...
    unmapped = 1 << (ARCH_PAGE_SHIFT + ARCH_INDEX_WIDTH * (ARCH_PAGE_TBL_LEVELS - 1));
    ...
    {
        rt_ubase_t *mmu_l1_0 = top_table + lvl_off[0];
        for (int h = 1; h < RT_CPUS_NR; h++)
        {
            rt_ubase_t *mmu_l1_h = (rt_ubase_t *)(top_table + (rt_ubase_t)(h * ARCH_PAGE_SIZE)) + lvl_off[0];
            *mmu_l1_h = *mmu_l1_0;
            rt_hw_cpu_dcache_clean(mmu_l1_h, sizeof(*mmu_l1_h));
        }
    }
