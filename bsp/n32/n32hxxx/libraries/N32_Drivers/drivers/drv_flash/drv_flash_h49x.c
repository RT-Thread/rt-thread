 ...
 * Change Logs:
 * Date           Author           Notes
 * 2026-08-07     li.mengmeng      implement for N32H49x dual-bank mode
 */
...
static rt_bool_t n32_flash_dual_bank_is_active(void)
{
    return FLASH_GetFlagSTS(FLASH_FLAG_BANKMODE) == SET ? RT_TRUE : RT_FALSE;
}

static rt_bool_t n32_flash_dcache_suspend(void)
{
    rt_bool_t enabled;

    enabled = (FLASH->AC & FLASH_AC_DCAHEN) != 0U ? RT_TRUE : RT_FALSE;
    if (enabled)
    {
        FLASH_DCacheCmd(FLASH_DCache_DIS);
        FLASH_DCacheRST();
    }
...
static void n32_flash_dcache_restore(rt_bool_t enabled)
{
    if (enabled)
    {
        FLASH_DCacheRST();
        FLASH_DCacheCmd(FLASH_DCache_EN);
    }
}
...
int n32_flash_read(rt_uint32_t addr, rt_uint8_t *buf, size_t size)
{
    ...
    dcache_enabled = n32_flash_dcache_suspend();
    rt_memcpy(buf, (const void *)addr, size);
    n32_flash_dcache_restore(dcache_enabled);
...
int n32_flash_write(rt_uint32_t addr, const rt_uint8_t *buf, size_t size)
{
    ...
    if (!n32_flash_dual_bank_is_active())
    {
        LOG_E("write rejected: Flash is not in dual-bank mode");
        return -RT_ERROR;
    }
...
int n32_flash_erase(rt_uint32_t addr, size_t size)
{
    ...
    if (!n32_flash_dual_bank_is_active())
    {
        LOG_E("erase rejected: Flash is not in dual-bank mode");
        return -RT_ERROR;
    }
...
static int n32_flash_init(void)
{
    ...
    result = rt_mutex_init(&_flash_mutex, "flash", RT_IPC_FLAG_PRIO);
    if ((result == RT_EOK) && !n32_flash_dual_bank_is_active())
    {
        LOG_E("single-bank mode detected; erase/write are disabled");
    }
