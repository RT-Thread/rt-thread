...
for i in range(len(opt)):
	...
	if is_opt("camera", inkey): q_camera = opt[i + 1]
...
if len(q_camera) > 0:
	if is_windows:
		print("Camera is supported only on Linux")
		exit(-1)
	camera_cfg = """ --device '{"driver": "vfio-user-pci","socket": {"path": "/tmp/qemu-device-camera.sock", "type": "unix"}}'"""
...
if len(q_camera) > 0:
	...
	os.system("{} ./qemu-device-camera.c {} &".format(shell, q_camera))

	import time
	start_time = time.time()
	while not os.path.exists("/tmp/qemu-device-camera.sock"):
	    if time.time() - start_time > 5:
	        print("qemu-device-camera timeout")
	        exit(-1)
	    time.sleep(1)

if len(q_dtbo) > 0:
	amp_mode = False
	q_bootargs = "rpmsg.mode=master " + q_bootargs
	# Read all dtbo
		...
		for dtsi in q_dtbo:
			with open(dtsi) as dtsi:
				...
				if not amp_mode and len(re.findall('amp_soc', dts_content)) > 0:
					amp_cfg += " -device loader,file=amp.dtb,addr={}".format(re.findall(r'dtb-entry\s*=\s*<(\S+)>;', dts_content)[0])
	...
	# Add Console for AMP
	amp_cfg += " -serial telnet:localhost:4323,server,nowait"
	# Add Storage for AMP
	q_amp_block = q_block + ".amp.qcow2"
	if not os.path.exists(q_amp_block):
		shutil.copyfile(q_block + ".qcow2", q_amp_block)
	amp_cfg += " -drive if=none,file={},format=qcow2,id=blk1 -device virtio-blk-device,drive=blk1".format(q_amp_block)
	# Keep vda0 is root
	cmd_base = cmd_base.replace(",drive=blk0", ",drive=blk0,bus=virtio-mmio-bus.0")
	# Build AMP dtb
	with open("amp.dt.h", 'w') as gic_inc:
		gic_inc.write("#define GIC_VERSION {}\n\r".format(q_gic))
		gic_inc.write("#define CPU_ID {}\n\r".format(q_smp - 1))
	dtc.dts_to_dtb(RTT_ROOT, ["amp.dts"], ignore_warning = ["simple_bus_reg", "unit_address_vs_reg", "clocks_is_cell", "gpios_property"])
	os.remove("amp.dt.h")
	# Dump raw QEMU dts
...
if len(q_camera) > 0:
	os.system("kill -15 `pidof qemu-device-camera`")
