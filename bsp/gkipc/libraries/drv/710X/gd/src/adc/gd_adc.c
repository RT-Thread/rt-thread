...
GERR GD_ADC_Init( void )
{
    ...
    {
        ADC_state_machine_data[i].channel       = 0;
    ...
    GH_ADC_set_AUX_ATOP(GD_ADC_AUX_ATOP_INITIALIZE);
    // 001 0 1 0
...
GERR GD_ADC_Open( GD_ADC_OPEN_PARAMS_S *openParamsP, GD_HANDLE* pHandle  )
{
    ...
    *pHandle = (GD_HANDLE)(&ADC_state_machine_data[openParamsP->channel]);
    ...
    {
        GH_ADC_set_AUX_ATOP_sar_maxsel(GD_ADC_CONTROL_CHANNEL_ONE);
        GH_ADC_set_Control_channel(GD_ADC_REQ_CHANNEL_ONE);
    }
    else
    {
        GH_ADC_set_AUX_ATOP_sar_maxsel(GD_ADC_CONTROL_CHANNEL_TWO);
    ...
    GD_INT_Enable(&ADC_state_machine_data[openParamsP->channel].adcHandle, GD_INT_ENABLED);
