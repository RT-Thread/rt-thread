...
Value	Description
[31:7]	reserved	R	±￡á?
[6]	Ssi2_cken	0x1	Ssi2 ????ê±?óê1?ü
[5]	ssi_cken	0x1	ssi ????ê±?óê1?ü
[4]	Sd2_cken	0x1	Sd2 ????ê±?óê1?ü
[3]	sd_cken	0x1	sd ????ê±?óê1?ü
[2]	reserved	0x1	±￡á?
[1]	eth_cken	0x1	eth ????ê±?óê1?ü
[0]	usb_cken	0x1	usb ????ê±?óê1?ü
*/
...
GERR GD_USB_DMA_Out_TranTx(U32 read_addr, U32 read_len)
{
    ...
    GH_USB_PHY_set_Tx_Config_tx_dma_blk(DMA_TBLK_SZ_BYTES_64);    // 等价于 dma 控制寄存器 BlockSize 位 : 64Bytes
    ...
    GH_DMA_set_Control(MUSB_TX_DMA_CHAN, data);
    GH_DMA_set_Status(MUSB_TX_DMA_CHAN, 0);
    GH_USB_PHY_set_Tx_Config_tx_dma_blk(DMA_TBLK_SZ_BYTES_32);    // 等价于 dma 控制寄存器 BlockSize 位 : 32Bytes
    ...
    GH_DMA_set_Control(MUSB_TX_DMA_CHAN, data);
    GH_DMA_set_Status(MUSB_TX_DMA_CHAN, 0);
    GH_USB_PHY_set_Tx_Config_tx_dma_blk(DMA_TBLK_SZ_BYTES_16);    // 等价于 dma 控制寄存器 BlockSize 位 : 16Bytes
    ...
    GH_DMA_set_Control(MUSB_TX_DMA_CHAN, data);
    GH_DMA_set_Status(MUSB_TX_DMA_CHAN, 0);
    GH_USB_PHY_set_Tx_Config_tx_dma_blk(DMA_TBLK_SZ_BYTES_8);    // 等价于 dma 控制寄存器 BlockSize 位 : 8Bytes
