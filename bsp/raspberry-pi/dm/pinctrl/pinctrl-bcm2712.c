...
#define DBG_TAG "pinctrl.bcm2712"
...
#define GPIO_REGS(n, mr, mb, pr, pb)    [n] = { ((mr)*4)*8 + (mb)*4, ((pr)*4)*8 + (pb)*2 }
#define EMMC_REGS(n, r, b)              [n] = { 0, ((r)*4)*8 + (b)*2 }
#define AGPIO_REGS(n, mr, mb, pr, pb)   [n] = { ((mr)*4)*8 + (mb)*4, ((pr)*4)*8 + (pb)*2 }
#define SGPIO_REGS(n, mr, mb)           [n+32] = { ((mr)*4)*8 + (mb)*4, REG_BIT_INVALID }

#define GPIO_PIN(a)                     { .number = a,    .name = "gpio" #a }
#define AGPIO_PIN(a)                    { .number = a,    .name = "aon_gpio" #a }
#define SGPIO_PIN(a)                    { .number = a+32, .name = "aon_sgpio" #a }
...
#define PIN(i, f1, f2, f3, f4, f5, f6, f7, f8) \
    ...
    rt_uint8_t funcs[BCM2712_FSEL_COUNT - 1];
};
...
struct bcm2712_pinctrl_soc_data
{
    rt_size_t num_pins;
    const struct bcm2712_pin_desc *desc;
    const struct bcm2712_pin_funcs *funcs;
    const struct bcm2712_pin_regs *regs;
};

struct bcm2712_pinctrl
{
    struct rt_device_pin parent;
    const struct bcm2712_pinctrl_soc_data *soc_data;

    void *base;
    struct rt_spinlock lock;
};

#define raw_to_bcm2712_pinctrl(raw) rt_container_of(raw, struct bcm2712_pinctrl, parent)
...
#define FUNC_NAME(name) [func_##name] = #name
    ...
    FUNC_NAME(alt7),
    FUNC_NAME(alt8),
    FUNC_NAME(aon_cpu_standbyb),
    FUNC_NAME(aon_fp_4sec_resetb),
    FUNC_NAME(aon_gpclk),
    FUNC_NAME(aon_pwm),
    FUNC_NAME(arm_jtag),
    FUNC_NAME(aud_fs_clk0),
    FUNC_NAME(avs_pmu_bsc),
    FUNC_NAME(bsc_m0),
    FUNC_NAME(bsc_m1),
    FUNC_NAME(bsc_m2),
    FUNC_NAME(bsc_m3),
    FUNC_NAME(clk_observe),
    FUNC_NAME(ctl_hdmi_5v),
    FUNC_NAME(enet0),
    FUNC_NAME(enet0_mii),
    FUNC_NAME(enet0_rgmii),
    FUNC_NAME(ext_sc_clk),
    FUNC_NAME(fl0),
    FUNC_NAME(fl1),
    FUNC_NAME(gpclk0),
    FUNC_NAME(gpclk1),
    FUNC_NAME(gpclk2),
    FUNC_NAME(hdmi_tx0_auto_i2c),
    FUNC_NAME(hdmi_tx0_bsc),
    FUNC_NAME(hdmi_tx1_auto_i2c),
    FUNC_NAME(hdmi_tx1_bsc),
    FUNC_NAME(i2s_in),
    FUNC_NAME(i2s_out),
    FUNC_NAME(ir_in),
    FUNC_NAME(mtsif),
    FUNC_NAME(mtsif_alt),
    FUNC_NAME(mtsif_alt1),
    FUNC_NAME(pdm),
    FUNC_NAME(pkt),
    FUNC_NAME(pm_led_out),
    FUNC_NAME(sc0),
    FUNC_NAME(sd0),
    FUNC_NAME(sd2),
    FUNC_NAME(sd_card_a),
...
rt_inline rt_uint32_t bcm2712_reg_rd(struct bcm2712_pinctrl *pc, int reg)
{
    return HWREG32(pc->base + reg);
}

rt_inline void bcm2712_reg_wr(struct bcm2712_pinctrl *pc, int reg, rt_uint32_t val)
{
    HWREG32(pc->base + reg) = val;
}

static void bcm2712_pinctrl_fsel_set(struct bcm2712_pinctrl *pc, rt_ubase_t pin,
        enum bcm2712_funcs func)
{
    int fsel, cur;
    const rt_uint8_t *pin_funcs;
    rt_uint32_t bit = pc->soc_data->regs[pin].mux_bit, val;
    ...
    if (func >= BCM2712_FSEL_COUNT)
    {
        ...
        pin_funcs = pc->soc_data->funcs[pin].funcs;

        for (int i = 1; i < BCM2712_FSEL_COUNT; ++i)
        {
...
static void bcm2712_pull_config_set(struct bcm2712_pinctrl *pc, rt_ubase_t pin,
        rt_uint32_t arg)
{
    rt_uint32_t bit = pc->soc_data->regs[pin].pad_bit, val;
...
static const struct rt_pin_ctrl_conf_params bcm2712_conf_params[] =
{
...
static rt_err_t bcm2712_pinctrl_confs_apply_once(struct bcm2712_pinctrl *pc,
        struct rt_ofw_node *np)
{
    ...
    enum bcm2712_funcs func = func_count;
    ...
    for (int i = 0; i < RT_ARRAY_SIZE(bcm2712_func_names); ++i)
    {
        if (!rt_strcmp(string, bcm2712_func_names[i]))
        {
            func = (enum bcm2712_funcs)i;
            break;
        }
    }
    ...
    for (int i = 0; i < RT_ARRAY_SIZE(bcm2712_conf_params); ++i)
    {
        if (rt_ofw_prop_read_bool(np, bcm2712_conf_params[i].propname))
        {
            conf = &bcm2712_conf_params[i];
            break;
        }
    }
    ...
    {
        pin = pc->soc_data->num_pins;

        for (int i = 0; i < pc->soc_data->num_pins; ++i)
        {
            if (!rt_strcmp(pc->soc_data->desc[i].name, string))
            {
                pin = pc->soc_data->desc[i].number;
                break;
            }
        }
        ...
        bcm2712_pinctrl_fsel_set(pc, pin, func);
...
static rt_err_t bcm2712_pinctrl_confs_apply(struct rt_device *device, void *fw_conf_np)
{
    rt_err_t err;
    rt_bool_t stage2 = RT_FALSE;
    struct rt_ofw_node *conf_np = fw_conf_np, *child;
    struct bcm2712_pinctrl *pc = raw_to_bcm2712_pinctrl(device);

    rt_hw_spin_lock(&pc->lock.lock);

    rt_ofw_foreach_child_node(conf_np, child)
    {
        stage2 = RT_TRUE;

        if ((err = bcm2712_pinctrl_confs_apply_once(pc, child)))
        {
            goto _unlock;
        }
    }
    ...
    if (!stage2)
    {
        err = bcm2712_pinctrl_confs_apply_once(pc, conf_np);
    }

_unlock:
    rt_hw_spin_unlock(&pc->lock.lock);

    return err;
}

static const struct rt_pin_ops bcm2712_pinctrl_ops =
{
    .pin_ctrl_confs_apply = bcm2712_pinctrl_confs_apply,
};

static rt_err_t bcm2712_pinctrl_probe(struct rt_platform_device *pdev)
{
    rt_err_t err;
    struct rt_device *dev = &pdev->parent;
    struct bcm2712_pinctrl *pc = rt_calloc(1, sizeof(*pc));

    if (!pc)
    {
        return -RT_ENOMEM;
    }
    pc->soc_data = pdev->id->data;
    ...
    rt_spin_lock_init(&pc->lock);
    pc->parent.ops = &bcm2712_pinctrl_ops;
...
static const struct bcm2712_pin_desc bcm2712_c0_gpio_pins[] =
{
    GPIO_PIN(0),
    GPIO_PIN(1),
    GPIO_PIN(2),
...
static struct bcm2712_pinctrl_soc_data bcm2712_c0_data =
{
    .num_pins = RT_ARRAY_SIZE(bcm2712_c0_gpio_pins),
    .desc = bcm2712_c0_gpio_pins,
    .funcs = bcm2712_c0_gpio_pin_funcs,
    .regs = bcm2712_c0_gpio_pin_regs,
};

static struct bcm2712_pin_desc bcm2712_c0_aon_gpio_pins[] =
{
    AGPIO_PIN(0),
    AGPIO_PIN(1),
    AGPIO_PIN(2),
...
static const struct bcm2712_pin_regs bcm2712_c0_aon_gpio_pin_regs[] =
{
...
static struct bcm2712_pinctrl_soc_data bcm2712_c0_aon_data =
{
    .num_pins = RT_ARRAY_SIZE(bcm2712_c0_aon_gpio_pins),
    .desc = bcm2712_c0_aon_gpio_pins,
    .funcs = bcm2712_c0_aon_gpio_pin_funcs,
    .regs = bcm2712_c0_aon_gpio_pin_regs,
};

static const struct bcm2712_pin_desc bcm2712_d0_gpio_pins[] =
{
    GPIO_PIN(1),
    GPIO_PIN(2),
    GPIO_PIN(3),
...
static const struct bcm2712_pin_regs bcm2712_d0_gpio_pin_regs[] =
{
    GPIO_REGS(1, 0, 0, 4, 5),
    GPIO_REGS(2, 0, 1, 4, 6),
    GPIO_REGS(3, 0, 2, 4, 7),
...
static struct bcm2712_pinctrl_soc_data bcm2712_d0_data =
{
    .num_pins = RT_ARRAY_SIZE(bcm2712_d0_gpio_pins),
    .desc = bcm2712_d0_gpio_pins,
    .funcs = bcm2712_d0_gpio_pin_funcs,
    .regs = bcm2712_d0_gpio_pin_regs,
};

static struct bcm2712_pin_desc bcm2712_d0_aon_gpio_pins[] =
{
    AGPIO_PIN(0),
    AGPIO_PIN(1),
    AGPIO_PIN(2),
...
static const struct bcm2712_pin_regs bcm2712_d0_aon_gpio_pin_regs[] =
{
...
static struct bcm2712_pinctrl_soc_data bcm2712_d0_aon_data =
{
    .num_pins = RT_ARRAY_SIZE(bcm2712_d0_aon_gpio_pins),
    .desc = bcm2712_d0_aon_gpio_pins,
    .funcs = bcm2712_d0_aon_gpio_pin_funcs,
    .regs = bcm2712_d0_aon_gpio_pin_regs,
};

static const struct rt_ofw_node_id bcm2712_pinctrl_ofw_ids[] =
{
    { .compatible = "brcm,bcm2712-pinctrl", .data = &bcm2712_c0_data },
    { .compatible = "brcm,bcm2712-aon-pinctrl", .data = &bcm2712_c0_aon_data },
    { .compatible = "brcm,bcm2712c0-pinctrl", .data = &bcm2712_c0_data },
    { .compatible = "brcm,bcm2712c0-aon-pinctrl", .data = &bcm2712_c0_aon_data },
    { .compatible = "brcm,bcm2712d0-pinctrl", .data = &bcm2712_d0_data },
    { .compatible = "brcm,bcm2712d0-aon-pinctrl", .data = &bcm2712_d0_aon_data },
    { /* sentinel */ }
};

static struct rt_platform_driver bcm2712_pinctrl_driver =
{
    .name = "pinctrl-bcm2712",
    .ids = bcm2712_pinctrl_ofw_ids,

    .probe = bcm2712_pinctrl_probe,
};

static int bcm2712_pinctrl_register(void)
{
    rt_platform_driver_register(&bcm2712_pinctrl_driver);
