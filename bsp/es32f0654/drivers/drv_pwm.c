...
{
    /* available when pclk1 more then 1MHz */
    uint64_t _arr = (uint64_t)cmu_get_pclk1_clock() / 1000000 * ns /
                    (timer_initstruct->init.prescaler + 1) / 1000 - 1;
...
{
    /* available when pclk1 more then 1MHz */
    uint64_t tmp = (uint64_t)cmu_get_pclk1_clock() / 1000000 * ns /
                   (timer_initstruct->init.prescaler + 1) / 1000 - 1;
