...
GERR GD_CRYPTO_Open(GD_CRYPTO_OpenParamsT* pOpenParams, GD_HANDLE* pHandle)
{
    ...
    case GD_CRYPTO_DES:
        ...
        for(i=0;i<nkey;i++)
        {
            GH_CRYPTO_set_DES_Key(nkey-i-1,(*pOpenParams->pkey).key[i]);
        }
    ...
    case GD_CRYPTO_AES_128:
        ...
        for(i=0;i<nkey;i++)
        {
            GH_CRYPTO_set_AES_128_Key(nkey-i-1,(*pOpenParams->pkey).key[i]);
        }
    ...
    case GD_CRYPTO_AES_192:
        ...
        for(i=0;i<nkey;i++)
        {
            GH_CRYPTO_set_AES_192_Key(nkey-i-1,(*pOpenParams->pkey).key[i]);
        }
        aesdevice.mode      = pOpenParams->mode;
        aesdevice.useint    = pOpenParams->useint;
        aesdevice.notifyFct = pOpenParams->notifyFct;
    ...
    case GD_CRYPTO_AES_256:
        ...
        for(i=0;i<nkey;i++)
        {
            GH_CRYPTO_set_AES_256_Key(nkey-i-1,(*pOpenParams->pkey).key[i]);
        }
        aesdevice.mode      = pOpenParams->mode;
        aesdevice.useint    = pOpenParams->useint;
        aesdevice.notifyFct = pOpenParams->notifyFct;
...
static GERR gd_crypto_ProcessData(GD_HANDLE handle, U32 type, GD_CRYPTO_DATA_T* data_in, GD_CRYPTO_DATA_T* data_out)
{
    ...
    case GD_CRYPTO_DES:
        ...
        GH_CRYPTO_set_DES_Input(1, data_in->des_data.data_hi);
        GH_CRYPTO_set_DES_Input(0, data_in->des_data.data_lo);
        if(device->useint)
        {
            do
            {
            ...
            data_out->des_data.data_hi = GH_CRYPTO_get_DES_Output(1);
            data_out->des_data.data_lo = GH_CRYPTO_get_DES_Output(0);
        }
        else
        {
    ...
    case GD_CRYPTO_AES_256:
        ...
        GH_CRYPTO_set_AES_Input(3, data_in->aes_data.data_127_96);
        GH_CRYPTO_set_AES_Input(2, data_in->aes_data.data_95_64);
        GH_CRYPTO_set_AES_Input(1, data_in->aes_data.data_63_32);
        GH_CRYPTO_set_AES_Input(0, data_in->aes_data.data_31_0);
        if(device->useint)
        {
            do
            {
            ...
            data_out->aes_data.data_127_96 = GH_CRYPTO_get_AES_Output(3);
            data_out->aes_data.data_95_64  = GH_CRYPTO_get_AES_Output(2);
            data_out->aes_data.data_63_32  = GH_CRYPTO_get_AES_Output(1);
            data_out->aes_data.data_31_0   = GH_CRYPTO_get_AES_Output(0);
        }
        else
        {
