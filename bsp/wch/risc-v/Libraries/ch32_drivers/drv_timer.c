...
static struct ch32_clock_timer ch32_clock_timer_obj[] =
{
#ifdef BSP_USING_TIM1
    TIM1_CONFIG,
#endif
...
static void ch32_clock_timer_init(struct rt_clock_timer_device *timer, rt_uint32_t state)
{
    RT_ASSERT(timer != RT_NULL);
    TIM_HandleTypeDef *tim = RT_NULL;
    RCC_ClocksTypeDef RCC_ClockStruct;
    NVIC_InitTypeDef NVIC_InitStruct;
    struct ch32_clock_timer *tim_device = RT_NULL;
    ...
    if(state)
    {
        ...
        tim_device = (struct ch32_clock_timer *)timer;
...
static rt_err_t ch32_clock_timer_start(struct rt_clock_timer_device *timer, rt_uint32_t cnt, rt_clock_timer_mode_t mode)
{
    RT_ASSERT(timer != RT_NULL);
    TIM_HandleTypeDef *tim = RT_NULL;
    tim = (TIM_HandleTypeDef *)timer->parent.user_data;
...
static void ch32_clock_timer_stop(struct rt_clock_timer_device *timer)
{
    RT_ASSERT(timer != RT_NULL);
    TIM_HandleTypeDef *tim = RT_NULL;
...
static rt_uint32_t ch32_clock_timer_count_get(struct rt_clock_timer_device *timer)
{
    RT_ASSERT(timer != RT_NULL);
    TIM_HandleTypeDef *tim = RT_NULL;
    tim = (TIM_HandleTypeDef *)timer->parent.user_data;
...
static rt_err_t ch32_clock_timer_control(struct rt_clock_timer_device *timer, rt_uint32_t cmd, void *args)
{
    RT_ASSERT(timer != RT_NULL);
...
#error " unsupported CH32 series! "
    ...
    {
        if (*(rt_clock_timer_mode_t *)args == CLOCK_TIMER_MODE_ONESHOT)
        {
            /* set timer to single mode */
            tim->instance->CTLR1 &= (rt_uint16_t) ~((rt_uint16_t)TIM_OPM);
            tim->instance->CTLR1 |= TIM_OPMode_Single;
    ...
    {
        *(rt_clock_timer_mode_t *)args = tim->instance->CNT;
        break;
    }

    case CLOCK_TIMER_CTRL_STOP:
    {
        ch32_clock_timer_stop(timer);
        break;
    }
...
static const struct rt_clock_timer_info ch32_clock_timer_info = TIM_DEV_INFO_CONFIG;
static const struct rt_clock_timer_ops ch32_clock_timer_ops =
{
    ch32_clock_timer_init,
    ch32_clock_timer_start,
    ch32_clock_timer_stop,
    ch32_clock_timer_count_get,
    ch32_clock_timer_control
};

static void ch32_clock_timer_isr(struct rt_clock_timer_device *device)
{
    RT_ASSERT(device != RT_NULL);
    struct ch32_clock_timer *clock_timer = RT_NULL;
    clock_timer = rt_container_of(device, struct ch32_clock_timer, device);

    if(TIM_GetITStatus(clock_timer->handle.instance, TIM_IT_Update) != RESET)
    {
        rt_clock_timer_isr(device);
        TIM_ClearITPendingBit(clock_timer->handle.instance, TIM_IT_Update);
    }
}
...
void TIM1_UP_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM1_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM2_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM2_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM3_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM3_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM4_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM4_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM5_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM5_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM6_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM6_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM7_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM7_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM8_UP_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM8_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM9_UP_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM9_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
void TIM10_UP_IRQHandler(void)
{
    GET_INT_SP();
    rt_interrupt_enter();
    ch32_clock_timer_isr(&(ch32_clock_timer_obj[TIM10_INDEX].device));
    rt_interrupt_leave();
    FREE_INT_SP();
}
...
static int rt_hw_timer_init(void)
{
    ...
    for (i = 0; i < sizeof(ch32_clock_timer_obj) / sizeof(ch32_clock_timer_obj[0]); i++)
    {
        ch32_clock_timer_obj[i].device.info = &ch32_clock_timer_info;
        ch32_clock_timer_obj[i].device.ops  = &ch32_clock_timer_ops;
        result = rt_clock_timer_register(&ch32_clock_timer_obj[i].device,
                    ch32_clock_timer_obj[i].name, (void *)&ch32_clock_timer_obj[i].handle);
