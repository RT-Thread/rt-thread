...
/*
 * AddressSanitizer: bulk memory helpers are not instrumented in this version.
 * ASan-aware replacements with explicit source/destination range checks are
 * deferred to a later phase. Calls to these helpers can therefore miss heap
 * buffer overflows and use-after-free accesses, even from instrumented code.
 */
#ifdef RT_USING_ASAN
