...
#define CLIC_INT_ATTR_MODE_MASK             GENMASK(7, CLIC_INT_ATTR_MODE_SHIFT)
#define CLIC_INT_ATTR_MODE_MACHINE          (0x3 << CLIC_INT_ATTR_MODE_SHIFT)
...
#define CLIC_INT_CTL_BITS_MAX               8
...
static void riscv_clic_irq_mask_raw(struct riscv_clic *clic, int hwirq)
{
    ...
    if (hwirq >= clic->ipi_base)
    {
        ...
        rt_hw_atomic_and(ipi_enable, ~BIT(hwirq - clic->ipi_base));

        if (rt_hw_atomic_load(ipi_enable) != 0)
        {
            return;
        }
    ...
    HWREG8(clic->regs + CLIC_INT_IE + 4 * hwirq) = 0;
}
...
static void riscv_clic_irq_unmask_raw(struct riscv_clic *clic, int hwirq)
{
    ...
    if (hwirq >= clic->ipi_base)
    {
        ...
        rt_hw_atomic_or(ipi_enable, BIT(hwirq - clic->ipi_base));
        hwirq = CLIC_INT_ID_xSIP;
    }
#endif /* RT_USING_SMP */
...
static void riscv_clic_irq_send_ipi_raw(struct riscv_clic *clic, int ipi, rt_bitmap_t *cpumask)
{
    ...
    RT_ASSERT(ipi >= clic->ipi_base && ipi < clic->ipi_base + RT_MAX_IPI);
    ipi -= clic->ipi_base;
    ...
    rt_bitmap_for_each_set_bit(cpumask, cpu, RT_CPUS_NR)
    {
        ...
        rt_hw_atomic_or(&clic->ipi_pending[cpu], BIT(ipi));
        ...
        rt_bool_t (*callback)(int hwirq, void *data), void *data)
{
    ...
    else if (hwirq == CLIC_INT_ID_xSIP)
    {
        ...
        pending = rt_hw_atomic_exchange(&clic->ipi_pending[rt_hw_cpu_id()], 0);

        rt_bitmap_for_each_set_bit(&pending, ipi, RT_MAX_IPI)
        {
            res = callback(clic->ipi_base + ipi, data) || res;
        }
...
static void riscv_clic_percpu_common_init(struct riscv_clic *clic)
{
    if (clic->shv)
    {
        csr_write(CSR_TVT, &fast_irq_vectors);
    }
}
...
static struct riscv_clic *riscv_clic_common_init(rt_ubase_t clic_base, rt_size_t clic_size,
        rt_bitmap_t quirks)
{
    rt_uint32_t cfg, value, level_bits;
#ifdef PIC_DISABLE_DM
    static struct riscv_clic non_dm_clic;
    struct riscv_clic *clic = &non_dm_clic;

    rt_memset(clic, 0, sizeof(*clic));
#else
    ...
    cfg = HWREG32(clic->regs + CLIC_CFG);
    cfg &= ~(CLIC_CFG_LEVEL_MASK | CLIC_CFG_VECTORED_ENABLE);
    level_bits = clic->int_ctl_bits;
    if (level_bits > CLIC_INT_CTL_BITS_MAX)
    {
        level_bits = CLIC_INT_CTL_BITS_MAX;
    }
    cfg |= level_bits << CLIC_CFG_LEVEL_SHIFT;
    cfg |= CLIC_CFG_MODE_MASK;

    if (rt_bitmap_test_bit(&clic->quirks, CLIC_QUIRK_SHV))
    {
        cfg |= CLIC_CFG_VECTORED_ENABLE;
    }

    HWREG32(clic->regs + CLIC_CFG) = cfg;
...
#ifdef ARCH_RISCV_M_MODE
    value |= CLIC_INT_ATTR_MODE_MACHINE;
#else
    value |= CLIC_INT_ATTR_MODE_SUPERVISOR;
#endif

    for (int hwirq = 0; hwirq < clic->irqs_count; ++hwirq)
    {
        HWREG8(clic->regs + CLIC_INT_ATTR + 4 * hwirq) &=
                ~(CLIC_INT_ATTR_MODE_MASK | CLIC_INT_ATTR_TRIGGERED_MASK | CLIC_INT_ATTR_SHV);
...
static void riscv_clic_irq_ack(struct rt_pic_irq *pirq)
{
    ...
    riscv_clic_irq_ack_raw(clic, pirq->hwirq);
}
...
static void riscv_clic_irq_mask(struct rt_pic_irq *pirq)
{
    ...
    riscv_clic_irq_mask_raw(clic, pirq->hwirq);
}
...
static void riscv_clic_irq_unmask(struct rt_pic_irq *pirq)
{
    ...
    riscv_clic_irq_unmask_raw(clic, pirq->hwirq);
}

static rt_err_t riscv_clic_irq_set_priority(struct rt_pic_irq *pirq, rt_uint32_t priority)
{
    struct riscv_clic *clic = pirq->pic->priv_data;
    rt_uint32_t int_ctl_bits = rt_min_t(rt_uint32_t, clic->int_ctl_bits,
            CLIC_INT_CTL_BITS_MAX);
    ...
    priority = rt_min_t(rt_uint32_t, priority, BIT(int_ctl_bits) - 1);
    HWREG8(clic->regs + CLIC_INT_CTL + pirq->hwirq * 4) =
            priority << (CLIC_INT_CTL_BITS_MAX - int_ctl_bits);
...
static rt_err_t riscv_clic_ofw_init(struct rt_ofw_node *np, const struct rt_ofw_node_id *id)
{
    ...
    if (!(clic = riscv_clic_common_init(clic_base, clic_size, quirks)))
    {
        return rt_get_errno();
    }
...
static const struct rt_ofw_node_id riscv_clic_ofw_ids[] =
{
