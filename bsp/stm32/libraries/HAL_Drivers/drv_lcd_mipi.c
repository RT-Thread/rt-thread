...
static rt_err_t _mipi_write_cmd(struct rt_lcd_intf *device, void *p, rt_size_t num)
{
    rt_uint8_t *write;
    write = (rt_uint8_t *)p;
    struct lcd_mipi_intf *mipi_device;

    RT_ASSERT(device != RT_NULL);
    mipi_device = (struct lcd_mipi_intf *)device->parent.user_data;
...
static rt_err_t _mipi_intf_config(struct rt_lcd_intf *device, void *args)
{
    RT_ASSERT(device != RT_NULL);
...
int rt_lcd_intf_init(void)
{
    ...
    result = rt_lcd_intf_register(mipi_intf.intf, "lcd_intf", &mipi_ops, &mipi_intf);
    if (result != RT_EOK)
    {
