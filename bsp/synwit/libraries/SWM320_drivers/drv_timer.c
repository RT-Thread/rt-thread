...
static void swm_timer_configure(struct rt_clock_timer_device *timer_device, rt_uint32_t state)
{
    ...
    if (state)
    {
        ...
        TIMR_Init(clock_timer_cfg->TIMRx, TIMR_MODE_TIMER, SystemCoreClock, 1);
...
static rt_err_t swm_timer_start(rt_clock_timer_t *timer_device, rt_uint32_t cnt, rt_clock_timer_mode_t opmode)
{
    ...
    TIMR_SetPeriod(clock_timer_cfg->TIMRx, cnt);
...
static rt_err_t swm_timer_control(rt_clock_timer_t *timer_device, rt_uint32_t cmd, void *args)
{
    ...
    case CLOCK_TIMER_CTRL_FREQ_SET:
    {
        ...
        TIMR_Init(clock_timer_cfg->TIMRx, TIMR_MODE_TIMER, SystemCoreClock / freq, 1);
    }
    break;
    default:
    {
...
void TIMR5_Handler(void)
{
    /* enter interrupt */
    rt_interrupt_enter();
    swm_timer_isr(&(clock_timer_obj[TIM5_INDEX].time_device));
    /* leave interrupt */
    rt_interrupt_leave();
}
#endif //BSP_USING_TIM5
