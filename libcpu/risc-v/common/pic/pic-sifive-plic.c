...
static void plic_set_threshold_raw(struct plic_percpu *percpu, rt_uint32_t threshold)
{
    if (!percpu->valid)
    {
        return;
...
#if !defined(SOC_SPACEMIT_K1) || defined(PIC_DISABLE_DM)
...
static void plic_percpu_common_init(struct plic *plic)
{
    ...
    plic_set_threshold_raw(percpu, PLIC_ENABLE_THRESHOLD);
}
        ...
        rt_size_t ndev, rt_bitmap_t quirks)
{
    ...
    static struct plic non_dm_plic;
    struct plic *plic = &non_dm_plic;

    rt_memset(plic, 0, sizeof(*plic));
#else
...
#ifdef SOC_SPACEMIT_K1
    HWREG32(plic->regs + PRIORITY_BASE + pirq->hwirq * PRIORITY_PER_ID) =
            PLIC_ENABLE_THRESHOLD + 1;
#endif
}
...
static void plic_irq_mask(struct rt_pic_irq *pirq)
{
    ...
    /* K1 masks a live source by priority; keep its context enable bit set. */
    HWREG32(plic->regs + PRIORITY_BASE + pirq->hwirq * PRIORITY_PER_ID) =
...
static rt_bool_t plic_common_handler_isr(int hwirq, void *data)
{
    ...
    res = rt_pic_handle_isr(pirq) == RT_EOK;
