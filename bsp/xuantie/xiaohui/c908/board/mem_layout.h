 ...
 * +---------------+
 * |   kernel      |  linked at KERNEL_VADDR_START + MEM_PHYS_BASE + MEM_BOOT_SIZE
 * +---------------+ <- MEM_PHYS_BASE + MEM_BOOT_SIZE
 * |   boot shim   |  M-mode startup, VMA == LMA (physical)
...
/* system heap size, placed after .bss */
#ifndef MEM_HEAP_SIZE
#define MEM_HEAP_SIZE 0x2000000
#endif

#define MEM_KERNEL_SIZE (MEM_RTSMART_SIZE - MEM_BOOT_SIZE - MEM_GUARD_SIZE)

/* scratch slot used to pass the satp-mode probe
 * result from M-mode to the S-mode kernel */
#define MEM_BOOT_SCRATCH_OFF 0xFF00
