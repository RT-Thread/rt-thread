...
#define I2S_DEV_MAX_BUFF  (I2S_DEV_MAX_FRAME*512+DMA_BUFF_ADDR_ALIGN)
...
static U8 rxDmaBuffer[I2S_DEV_MAX_BUFF] __attribute__ ((section(".nocache_buffer")));
...
static GERR dma_open(SUB_DEVICE_S *dev)
{
	...
	{
		align_offset = DMA_BUFF_ADDR_ALIGN-((U32)rxDmaBuffer%DMA_BUFF_ADDR_ALIGN);
		dev->dmaFrameBuffer  = rxDmaBuffer+align_offset;
	...
	{
		align_offset = DMA_BUFF_ADDR_ALIGN-((U32)txDmaBuffer%DMA_BUFF_ADDR_ALIGN);
		dev->dmaFrameBuffer  = txDmaBuffer+align_offset;
	...
	for(i=0; i<dev->frameNumber; i++)
	{
		if(dev->subDevID == I2S_DEV_RX_ID)
		{
			dmaDescriptor.srcAddr  = REG_I2S_RX_DMA - 0x30000000;
		...
		else /* I2S_DEV_TX_ID */
		{
			...
			dmaDescriptor.dstAddr  = REG_I2S_TX_LEFT_DMA - 0x30000000;
