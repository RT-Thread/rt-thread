...
{
    {
        ...
        GD_ETH_PHY_LAN8700_SetWorkMode,     /* GD_ETH_PHY_SetWorkMode */
...
GERR GD_ETH_PHY_SetWorkMode(GD_HANDLE handle, GD_ETH_Work_ModeT workmode)
{
    ...
    else
    {
        ...
        if (workmode.bEnAutoNeg == GTRUE)
        {
            ...
            if(workmode.mode == GD_ETH_PHY_IF_MODE_RMII)
            {
                GH_GPIO_set_INPUT_CFG_in_sel(GD_GPIO_GET_IN_SEL(GD_GPIO_TYPE_INPUT_ENET_PHY_RXD_3) - 2, 0x01);
            }
        }
        else
        {
            ...
            else if (workmode.speed == GD_ETH_SPEED_10M )/* 10M */
            {
                ...
                if(workmode.mode == GD_ETH_PHY_IF_MODE_RMII)
                {
                    GH_GPIO_set_INPUT_CFG_in_sel(GD_GPIO_GET_IN_SEL(GD_GPIO_TYPE_INPUT_ENET_PHY_RXD_3) - 2, 0x00);
                }
            }
            else
            {
