...
static void _thread_exit(void)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(thread->spinlock));
    rt_timer_detach(&thread->thread_timer);
    /* insert to defunct thread list */
    rt_arch_spin_unlock_irqrestore(&(thread->spinlock), level);
    LOG_D("line:%d thread:%s exit\n",__LINE__,rt_thread_self()->parent.name);
...
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    ...
    level = rt_arch_spin_lock_irqsave(&(thread->spinlock)); //TODO need lock for cpu
...
rt_err_t rt_thread_wakeup(rt_thread_t thread)
{
    ...
    RT_ASSERT(thread != RT_NULL);
    RT_ASSERT(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread);
    temp = rt_arch_spin_lock_irqsave(&(thread->spinlock));
    func = thread->wakeup.func;
    thread->wakeup.func = RT_NULL;
    rt_arch_spin_unlock_irqrestore(&(thread->spinlock), temp);
    if (func)
    {
