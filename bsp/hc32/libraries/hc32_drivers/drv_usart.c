...
static void hc32_dma_config(struct rt_serial_device *serial, rt_ubase_t flag)
{
    ...
    if (RT_DEVICE_FLAG_DMA_RX == flag)
    {
        ...
        llp_init.u32Addr    = (uint32_t)&uart->llp_desc;
        DMA_LlpInit(uart_dma->Instance, uart_dma->channel, &llp_init);

        /* Configure LLP descriptor */
        uart->llp_desc.SARx  = dma_init.u32SrcAddr;
        uart->llp_desc.DARx  = dma_init.u32DestAddr;
        uart->llp_desc.DTCTLx = (dma_init.u32TransCount << DMA_DTCTL_CNT_POS) | (dma_init.u32BlockSize << DMA_DTCTL_BLKSIZE_POS);
        uart->llp_desc.LLPx  = (uint32_t)&uart->llp_desc;
        uart->llp_desc.CHCTLx = (dma_init.u32SrcAddrInc | dma_init.u32DestAddrInc | dma_init.u32DataWidth | \
