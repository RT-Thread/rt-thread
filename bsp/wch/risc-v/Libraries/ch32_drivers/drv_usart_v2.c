...
static void ch32_uart_init(struct ch32_uart_config *uart)
{
    ...
    GPIOInit(uart->rx_port, GPIO_Mode_IN_FLOATING, GPIO_Speed_50MHz, uart->rx_pin);
...
static rt_err_t ch32_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CLR_INT:
        ...
        {
            USART_ITConfig(uart->handle.Instance, USART_IT_TXE, DISABLE);
            USART_ITConfig(uart->handle.Instance, USART_IT_TC, DISABLE);
            USART_ClearFlag(uart->handle.Instance, USART_FLAG_TXE);
        }
#ifdef RT_SERIAL_USING_DMA
    ...
    case RT_DEVICE_CTRL_SET_INT:
        ...
        if (ctrl_arg & RT_DEVICE_FLAG_INT_TX)
        {
            USART_ITConfig(uart->handle.Instance, USART_IT_TXE, ENABLE);
...
static void uart_isr(struct rt_serial_device *serial)
{
    ...
    {
        char chr = uart->handle.Instance->DATAR & (rt_uint16_t)0xFF;
    ...
    {
        if (USART_GetITStatus(uart->handle.Instance, USART_IT_ORE) != RESET)
        {
        }

        if (USART_GetITStatus(uart->handle.Instance, USART_IT_NE) != RESET)
        {
        }

        if (USART_GetITStatus(uart->handle.Instance, USART_IT_FE) != RESET)
        {
        }

        if (USART_GetITStatus(uart->handle.Instance, USART_IT_PE) != RESET)
        {
        }

        if (USART_GetFlagStatus(uart->handle.Instance, USART_FLAG_TC) != RESET)
        {
            USART_ClearFlag(uart->handle.Instance, USART_FLAG_TC);
        }
...
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART1_RX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART1_TX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART2_RX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART2_TX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART3_RX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#endif /* defined(BSP_UART_USING_DMA_RX) && defined(BSP_USART3_RX_USING_DMA) */
#if defined(RT_SERIAL_USING_DMA) && defined(BSP_USART3_TX_USING_DMA)
#if defined (SOC_RISCV_SERIES_CH32V2)
...
#endif /* defined(BSP_UART_USING_DMA_TX) && defined(BSP_USART3_TX_USING_DMA) */
...
static void ch32_uart_get_config(void)
{
    ...
    uart_obj[UART1_INDEX].serial.config.rx_bufsz = BSP_USART1_RX_BUFSIZE;
    uart_obj[UART1_INDEX].serial.config.tx_bufsz = BSP_USART1_TX_BUFSIZE;
    uart_obj[UART1_INDEX].handle.Instance        = USART1;
    uart_obj[UART1_INDEX].uart_dma_flag          = 0;
#ifdef BSP_USART1_RX_USING_DMA
    uart_obj[UART1_INDEX].handle.HDMA_Rx          = &uart_obj[UART1_INDEX].dma_rx.handle;
    uart_obj[UART1_INDEX].serial.config.dma_ping_bufsz = BSP_USART1_DMA_PING_BUFSIZE;
    ...
    uart_obj[UART2_INDEX].serial.config.rx_bufsz = BSP_USART2_RX_BUFSIZE;
    uart_obj[UART2_INDEX].serial.config.tx_bufsz = BSP_USART2_TX_BUFSIZE;
    uart_obj[UART2_INDEX].handle.Instance        = USART2;
    uart_obj[UART2_INDEX].uart_dma_flag          = 0;
#ifdef BSP_USART2_RX_USING_DMA
    uart_obj[UART2_INDEX].handle.HDMA_Rx          = &uart_obj[UART2_INDEX].dma_rx.handle;
    uart_obj[UART2_INDEX].serial.config.dma_ping_bufsz = BSP_USART2_DMA_PING_BUFSIZE;
    ...
    uart_obj[UART3_INDEX].serial.config.rx_bufsz = BSP_USART3_RX_BUFSIZE;
    uart_obj[UART3_INDEX].serial.config.tx_bufsz = BSP_USART3_TX_BUFSIZE;
    uart_obj[UART3_INDEX].handle.Instance        = USART3;
    uart_obj[UART3_INDEX].uart_dma_flag          = 0;
#ifdef BSP_USART3_RX_USING_DMA
    uart_obj[UART3_INDEX].handle.HDMA_Rx          = &uart_obj[UART3_INDEX].dma_rx.handle;
    uart_obj[UART3_INDEX].serial.config.dma_ping_bufsz = BSP_USART3_DMA_PING_BUFSIZE;
...
static void HAL_DMA_IRQHandler(struct DMA_HandleTypeDef *hdma)
{
    ...
    if ((flag_it & 2u << (4 * channel_offset)))
    {
        ...
        dmax->INTFCR |= 1u << (4 * channel_offset);
    }
