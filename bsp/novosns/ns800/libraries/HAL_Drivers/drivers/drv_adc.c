...
static rt_err_t ns800_adc_hw_init(struct ns800_adc *adc)
{
    ...
    ADC_setPrescaler(adc->config->instance, ADC_CLK_DIV_4);
    ADC_setInterruptPulsePosMode(adc->config->instance, ADC_PULSE_END_OF_CONV);
...
static void ns800_adc_gpio_init(const struct ns800_adc_config *config)
{
    rt_uint32_t index;
    ...
    {
        GPIO_setPinConfig(config->pins[index].port,
                          config->pins[index].pin,
                          config->pins[index].mux);
        GPIO_setAnalogMode(config->pins[index].port,
                           config->pins[index].pin,
                           GPIO_ANALOG_ENABLED);
        GPIO_setPadConfig(config->pins[index].port,
                          config->pins[index].pin,
                          GPIO_PIN_TYPE_STD);
        GPIO_setQualificationMode(config->pins[index].port,
                                  config->pins[index].pin,
                                  GPIO_QUAL_ASYNC);
        GPIO_setDirectionMode(config->pins[index].port,
                              config->pins[index].pin,
                              GPIO_DIR_MODE_IN);
    }
}
...
static rt_err_t ns800_adc_config_sw_soc(struct ns800_adc *adc, rt_uint32_t channel)
{
    ...
    ns800_adc_setup_soc(adc, channel, channel, ADC_TRIGGER_SW_ONLY, NS800_ADC_SAMPLE_WINDOW);
...
static void ns800_adc_conv_irq_handler(void)
{
    ...
    for (index = 0U; index < NS800_ADC_INT_MAX; index++)
    {
        if ((ADC_getInterruptStatus(adc_obj.config->instance, (ADC_IntNumber)index) != 0U) &&
                (adc_irq_entries[index].callback != RT_NULL))
        {
                                     ...
                                     void *user_data)
{
    ...
    Interrupt_register(ADCA_CONV_IRQn, ns800_adc_conv_irq_handler);
    NVIC_ClearPendingIRQ(ADCA_CONV_IRQn);
    Interrupt_enable(ADCA_CONV_IRQn);
...
static rt_uint8_t ns800_adc_get_resolution(struct rt_adc_device *device)
{
    ...
    return NS800_ADC_RESOLUTION_BITS;
}
...
int rt_hw_adc_init(void)
{
    ...
    ns800_adc_gpio_init(adc_obj.config);
#ifdef RT_USING_DEVICE_OPS
