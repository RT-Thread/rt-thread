...
{
    pUSBCore = pCore;
    pCore->Info.CurrentStatus = USER_USER_USB_STATE_POWERED;
...
void USBDCore_MainRoutine(USBDCore_TypeDef *pCore)
{
    ...
    /*--------------------------------------------------------------------------------------------------------*/
    if ((pCore->Class.CallBack_MainRoutine.func != NULL) && (pCore->Info.CurrentStatus == USER_USER_USB_STATE_CONFIGURED))
    {
...
{
    return ((pCore->Info.CurrentStatus == USER_USER_USB_STATE_SUSPENDED) ? TRUE : FALSE);
}
...
static void _USBDCore_PowerHandler(USBDCore_TypeDef *pCore)
{
    ...
    if (pCore->Info.CurrentStatus == USER_USER_USB_STATE_SUSPENDED)
    {
        /*------------------------------------------------------------------------------------------------------*/
...
static void _USBDCore_Reset(USBDCore_TypeDef *pCore)
{
    ...
    pCore->Info.CurrentStatus = USER_USER_USB_STATE_DEFAULT;
...
static void _USBDCore_Suspend(USBDCore_TypeDef *pCore)
{
    ...
    /* turn off chip power.                                                                                   */
    /*--------------------------------------------------------------------------------------------------------*/
    if (pCore->Info.CurrentStatus >= USER_USER_USB_STATE_POWERED)
    {
        API_USB_POWER_OFF();
        pCore->Info.LastStatus = pCore->Info.CurrentStatus;
        pCore->Info.CurrentStatus = USER_USER_USB_STATE_SUSPENDED;
    }
...
static void _USBDCore_Standard_SetAddress(USBDCore_TypeDef *pCore)
{
    ...
    pCore->Device.Transfer.Action = USB_ACTION_DATAIN;
    pCore->Info.CurrentStatus = USER_USER_USB_STATE_ADDRESS;
...
static void _USBDCore_Standard_SetConfiguration(USBDCore_TypeDef *pCore)
{
    ...
    pCore->Info.CurrentStatus = USER_USER_USB_STATE_CONFIGURED;
