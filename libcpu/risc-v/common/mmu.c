...
static struct mmu_percpu_level_info percpu_level[RT_CPUS_NR][LEVEL_MAX + 1];
#else
#define PERCPU_OFFSET 0
#endif

rt_ubase_t MMUTable[(ARCH_PAGE_SIZE / sizeof(rt_ubase_t)) * RT_CPUS_NR]
        rt_align(ARCH_PAGE_SIZE);

static rt_uint8_t satp_level_4k, satp_level_2M;
static rt_ubase_t satp_mode;

struct page_table
{
    rt_ubase_t page[ARCH_PAGE_SIZE / sizeof(rt_ubase_t)];
};
static struct page_table *__init_page_array;

rt_inline rt_ubase_t mmu_attr_fixup(rt_ubase_t attr)
{
#if !defined(__HAS_MMU_VENDOR__) && __riscv_xlen != 32
    if (!riscv_isa_ext_test(SVPBMT))
    {
        attr &= ~PTE_VENDOR_MTMASK;
    }
#endif
...
rt_ubase_t get_free_page(void)
{
    static rt_ubase_t page_off;

    if (page_off < ARCH_PAGE_SIZE / sizeof(rt_ubase_t))
    {
        return (rt_ubase_t)__init_page_array[page_off++].page;
    }
...
{
    rt_ubase_t page, *table, off, level_shift;

    attr = mmu_attr_fixup(attr);
...
{
    rt_ubase_t page, off, level_shift;
    ...
    for (int level = 0; level < satp_level_4k; ++level)
    {
        ...
        tbl_shadow = (rt_ubase_t *)((tbl_shadow[off] >> PTE_BITS) << ARCH_PAGE_SHIFT);
 ...
 *
 * @note If ASID is supported (`ARCH_USING_ASID` is defined), the ASID is encoded
 *       into `satp`. Otherwise, switching address spaces invalidates the local TLB.
 */
#ifdef ARCH_USING_ASID
#define ASID_COUNT              (_UL(1) << SATP_ASID_BITS)
#define ASID_FIRST_GENERATION   ASID_COUNT

static rt_spinlock_t asid_lock = RT_SPINLOCK_INIT;

static rt_uint64_t _asid_acquire(rt_aspace_t aspace)
{
    static rt_uint32_t next_asid = 1;
    static rt_uint64_t global_asid_generation = ASID_FIRST_GENERATION;
    rt_ubase_t level;

    level = rt_spin_lock_irqsave(&asid_lock);

    if ((aspace->asid ^ global_asid_generation) >> SATP_ASID_BITS)
    {
        if (next_asid != ASID_COUNT)
        {
    ...
    rt_spin_unlock_irqrestore(&asid_lock, level);
...
{
    rt_ubase_t asid, page_table;

    page_table = (rt_ubase_t)rt_kmem_v2p(aspace->page_table + PERCPU_OFFSET);

    asid = _asid_acquire(aspace);

    csr_write(CSR_SATP, satp_mode | (asid << SATP_ASID_SHIFT) |
            (page_table >> ARCH_PAGE_SHIFT));
...
static int _map_percpu_area(rt_ubase_t *table, void *va, void *pa, int cpu_id)
{
    ...
    for (int level = 0; level < satp_level_4k; ++level)
    {
        if (!(page = (rt_ubase_t)rt_pages_alloc(0)))
        {
            return MMU_MAP_ERROR_NOPAGE;
        }
        ...
        percpu_level[cpu_id][level].page = table;
        percpu_level[cpu_id][level].offset = off;

        /* Step into the next level page table */
        table = (rt_ubase_t *)(((table[off] >> PTE_BITS) << ARCH_PAGE_SHIFT) - rt_kmem_pvoff());
    ...
    percpu_level[cpu_id][satp_level_4k].page = table;
    percpu_level[cpu_id][satp_level_4k].offset = off;

    return ARCH_PAGE_SIZE;
}

static void percpu_map_sync(struct rt_aspace *aspace, int level, rt_ubase_t *table, rt_ubase_t off)
{
    if (aspace == &rt_kernel_space &&
        percpu_level[0][level].page == table &&
        percpu_level[0][level].offset != off)
    {
        rt_ubase_t *table_shadow;
...
static void user_root_sync(struct rt_aspace *aspace, int level,
                           rt_ubase_t *table, rt_ubase_t off)
{
    if (aspace != &rt_kernel_space && level == 0 && table == aspace->page_table)
    {
        for (int i = 1; i < RT_CPUS_NR; ++i)
        {
            rt_ubase_t *table_shadow = (void *)aspace->page_table + i * ARCH_PAGE_SIZE;

            table_shadow[off] = table[off];
            rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, table_shadow + off,
                                 sizeof(*table_shadow));
        }
    }
}

static void pgtbl_entry_sync(struct rt_aspace *aspace, int level,
                             rt_ubase_t *table, rt_ubase_t off)
{
    percpu_map_sync(aspace, level, table, off);
    user_root_sync(aspace, level, table, off);
}
                        ...
                        size_t attr, size_t max_size)
{
    ...
    #ifdef MMU_PERCPU
        pgtbl_entry_sync(aspace, level, table, off);
    #endif

        /* Step into the next level page table */
        page = (table[off] >> PTE_BITS) << ARCH_PAGE_SHIFT;
        table = (rt_ubase_t *)(page - rt_kmem_pvoff());
...
#ifdef MMU_PERCPU
    pgtbl_entry_sync(aspace, max_level, table, off);
#endif
...
static size_t _unmap_area(struct rt_aspace *aspace, void *va, size_t max_size)
{
    int level, ref;
    size_t unmapped;
    rt_ubase_t entry, page, *table, *pos, off, level_shift;
    struct mmu_level_info level_info[LEVEL_MAX + 1];
    ...
    for (level = 0; level <= satp_level_4k; ++level)
    {
        ...
        pos = table + off;
        entry = *pos;
        unmapped = 1UL << (ARCH_PAGE_SHIFT +
                           VPN_BITS * (satp_level_4k - level));

        if (!(entry & PTE_V))
        {
            return max_size < unmapped ? max_size : unmapped;
        }

        level_info[level].pos = pos;

        if ((entry & PTE_ATTR_RWX) != 0)
        {
            break;
        }
...
#ifdef MMU_PERCPU
    pgtbl_entry_sync(aspace, level, pos - off, off);
#endif
    --level;
        ...
        if (pos)
        {
            ...
            if (ref == 1)
            {
                *pos = 0;
                ...
                if (pos >= table && pos < table + (ARCH_PAGE_SIZE / sizeof(rt_ubase_t)))
                {
                    pgtbl_entry_sync(aspace, level, table, pos - table);
                }
            #endif /* MMU_PERCPU */
            }
                       ...
                       rt_ubase_t *vtable, rt_ubase_t pv_off)
{
    ...
    rt_ubase_t aspace_start = KERNEL_VADDR_START;

#if !defined(RT_USING_SMART) && ARCH_RAM_OFFSET
    aspace_start = ARCH_RAM_OFFSET;
#endif
    ...
    /* The RV32 ioremap and MPR regions are placed below the kernel image. */
    aspace_start = (rt_ubase_t)v_address - rt_mpr_size;
#endif

    rt_aspace_init(aspace, (void *)aspace_start, 0UL - aspace_start, vtable);
...
{
    rt_ubase_t page, entry, off, level_shift, pa;
    rt_ubase_t va = (rt_ubase_t)vaddr;
    rt_ubase_t *table = tbl;

#ifdef MMU_PERCPU
    if (tbl == rt_kernel_space.page_table)
    {
        table = (rt_ubase_t *)(tbl + PERCPU_OFFSET);
    }
#endif /* MMU_PERCPU */
...
void rt_hw_mmu_setup(rt_aspace_t aspace, struct mem_desc *mdesc, int desc_nr)
{
    ...
    rt_hw_mmu_tbl_set((rt_ubase_t)rt_kernel_space.page_table);
    rt_page_cleanup();
}
...
void rt_hw_mem_setup_early(rt_ubase_t vaddr_start, rt_ubase_t paddr_start, void *pgtbl)
{
    ...
    rt_size_t map_pages;
    rt_ubase_t *early_pgtbl = pgtbl, paddr_end;
    ...
    map_pages = (size + (paddr_start & SUPPER_PAGE_MASK) + SUPPER_PAGE_MASK) /
            SUPPER_PAGE_SIZE;
...
_map_vaddress:
    ...
    while (map_pages--)
    {
        ...
        rt_ubase_t next_paddr = paddr_start + SUPPER_PAGE_SIZE;
        rt_ubase_t next_vaddr = vaddr_start + SUPPER_PAGE_SIZE;

        /* RV32 cannot represent an exclusive end address at 4 GiB. */
        if (next_paddr < paddr_start || next_vaddr < vaddr_start)
        {
            break;
        }

        paddr_start = next_paddr;
...
{
    rt_size_t alloc_bits = 0;
    rt_ubase_t *mmu_table;

#ifdef MMU_PERCPU
    alloc_bits = rt_page_bits(RT_CPUS_NR * ARCH_PAGE_SIZE);
#endif
    mmu_table = (rt_ubase_t *)rt_pages_alloc_ext(alloc_bits, PAGE_ANY_AVAILABLE);
    if (!mmu_table)
    {
        return RT_NULL;
    }
    ...
    {
        rt_memcpy((void *)mmu_table + i * ARCH_PAGE_SIZE,
                  (void *)rt_kernel_space.page_table + i * ARCH_PAGE_SIZE,
                  ARCH_PAGE_SIZE);
    }
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, mmu_table,
                         RT_CPUS_NR * ARCH_PAGE_SIZE);
#else
...
void rt_hw_mmu_pgtbl_delete(void *pgtbl)
{
    ...
    alloc_bits = rt_page_bits(RT_CPUS_NR * ARCH_PAGE_SIZE);
#endif
    rt_pages_free(pgtbl, alloc_bits);
}
