 ...
 /*
 * Copyright (c) 2006-2030, RT-Thread Development Team
 *
...
static struct n32_pwm n32_pwm_obj[] =
    {
...
#ifdef BSP_USING_TIM1_PWM
        {
            ...
            .ch[0].pin = GPIO_PIN_9,
            .ch[0].pwm_mode = TIM_OCMODE_PWM1,
            .ch[0].info = "TIM1 PWMCH0 PE9",
#endif /* BSP_USING_TIM1_PWM_CH0 */
#ifdef BSP_USING_TIM1_PWM_CH1
            .ch[1].gpio_grp = GPIOE,
            .ch[1].pin = GPIO_PIN_11,
            .ch[1].pwm_mode = TIM_OCMODE_PWM1,
            .ch[1].info = "TIM1 PWMCH1 PE11",
#endif /* BSP_USING_TIM1_PWM_CH1 */
#ifdef BSP_USING_TIM1_PWM_CH2
            .ch[2].gpio_grp = GPIOE,
            .ch[2].pin = GPIO_PIN_13,
            .ch[2].pwm_mode = TIM_OCMODE_PWM1,
            .ch[2].info = "TIM1 PWMCH2 PE13",
#endif /* BSP_USING_TIM1_PWM_CH2 */
#ifdef BSP_USING_TIM1_PWM_CH3
            .ch[3].gpio_grp = GPIOE,
            .ch[3].pin = GPIO_PIN_14,
            .ch[3].pwm_mode = TIM_OCMODE_PWM1,
            .ch[3].info = "TIM1 PWMCH3 PE14",
#endif /* BSP_USING_TIM1_PWM_CH3 */
#endif /*TIM1_REMAP_3*/
        },
 ...
 * @param  gpio_grp         
 */
static void n32_gpio_rcc_enable(GPIO_Module *gpio_grp)
{
 ...
 */
static  void n32_time_rcc_config(TIM_Module *htim)
{
...
static rt_uint32_t tim_clock_get(TIM_Module *htim)
{
     ...
     rt_uint32_t timclk;
     rt_uint32_t cfgr;
     rt_uint32_t apb1_pre;
     rt_uint32_t apb2_pre;
     RCC_GetClocksFreqValue(&RCC_Clocks);
     /* Read APB1/APB2 prescaler configuration from RCC->CFG.
      * On this SoC, timer clock is doubled only when APB prescaler != 1.
      * APB1 prescaler bits: CFG[10:8], APB2 prescaler bits: CFG[13:11].
      * Prescaler encoding: 0..3 => HCLK not divided (prescaler = 1),
      *                     4..7 => division by 2,4,8,16 and timer clock = 2 * PCLK.
      */
     cfgr = RCC->CFG;
     apb1_pre = (cfgr >> 8) & 0x7U;
     apb2_pre = (cfgr >> 11) & 0x7U;
     if (htim == TIM1 || htim == TIM8)
     {
         timclk = RCC_Clocks.Pclk2Freq;
         if (apb2_pre >= 4U)
         {
             timclk *= 2U;
         }
...
static rt_err_t drv_pwm_control(struct rt_device_pwm *device, int cmd, void *arg)
{
    ...
    case PWM_CMD_ENABLE:
        ...
        {
            return drv_pwmn_enable(pwm, configuration->channel, RT_TRUE);
        }
        else
        {
     ...
     case PWM_CMD_DISABLE:
         if (configuration->complementary)
         {
             return drv_pwmn_enable(pwm, configuration->channel, RT_FALSE);
         }
         else
         {
