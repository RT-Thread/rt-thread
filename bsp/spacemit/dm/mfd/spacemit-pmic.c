...
static void spacemit_pmic_irq_mask(struct rt_pic_irq *pirq)
{
    ...
    err = spacemit_pmic_update_bits(irqchip->pmic, value, irq_map->mask,
            irqchip->mask_unmask_non_inverted ? 0 : irq_map->mask);

    if (err)
    {
        LOG_E("Mask PMIC IRQ %d failed: %s", pirq->hwirq, rt_strerror(err));
    }
}
...
static void spacemit_pmic_irq_unmask(struct rt_pic_irq *pirq)
{
    ...
    if (err)
    {
        LOG_E("Unmask PMIC IRQ %d failed: %s", pirq->hwirq, rt_strerror(err));
    }
}
...
static rt_uint32_t spacemit_pmic_irq_valid_mask(struct spacemit_pmic_irqchip *irqchip,
        int reg_index)
{
    rt_uint32_t mask = 0;
    ...
    {
        if (irqchip->irqs_map[i].reg_offset / irqchip->reg_stride == reg_index)
        {
            mask |= irqchip->irqs_map[i].mask;
        }
    }
...
{
    rt_bool_t parent_rearmed;
    rt_uint32_t enabled, *status;
    ...
    while (RT_TRUE)
    {
        if (rt_sem_take(&irqchip->irq_sem, RT_WAITING_FOREVER))
        {
            continue;
        }

        parent_rearmed = RT_FALSE;
        ++irqchip->worker_count;
        ...
        for (int i = 0; i < irqchip->num_regs; ++i)
        {
            ...
            enabled = spacemit_pmic_read(pmic,
                    irqchip->mask_base + i * irqchip->reg_stride);

            if ((rt_err_t)enabled < 0)
            {
                LOG_E("Read IRQ mask failed error = %s", rt_strerror(enabled));

                goto _end;
            }

            if (irqchip->mask_unmask_non_inverted)
            {
                status[i] &= enabled;
            }
            else
        ...
        /* The PMIC line is clear after ACK; rearm before child handlers can schedule. */
        rt_hw_interrupt_umask(pmic->irq);
        ++irqchip->rearm_count;
        parent_rearmed = RT_TRUE;
...
static void spacemit_pmic_isr(int irqno, void *param)
{
    ...
    ++irqchip->hard_irq_count;
    rt_hw_interrupt_mask(irqchip->pmic->irq);

    rt_sem_release(&irqchip->irq_sem);
}

#define spacemit_pmic_to_i2c_client(pmic) rt_container_of((pmic)->dev, struct rt_i2c_client, parent)

static unsigned spacemit_pmic_i2c_read_unlocked(struct spacemit_pmic *pmic, unsigned reg)
{
    rt_uint8_t data = 0;
...
static rt_err_t spacemit_pmic_i2c_write_unlocked(struct spacemit_pmic *pmic,
...
{
    unsigned value;
    rt_err_t err = rt_mutex_take(pmic->io_lock, RT_WAITING_FOREVER);
    ...
    value = spacemit_pmic_i2c_read_unlocked(pmic, reg);
    rt_mutex_release(pmic->io_lock);
        ...
        unsigned reg, unsigned data)
{
    ...
    err = spacemit_pmic_i2c_write_unlocked(pmic, reg, data);
    rt_mutex_release(pmic->io_lock);
        ...
        unsigned reg, unsigned mask, unsigned data)
{
    ...
    if ((err = rt_mutex_take(pmic->io_lock, RT_WAITING_FOREVER)))
    {
        return err;
    }

    val = (rt_int32_t)spacemit_pmic_i2c_read_unlocked(pmic, reg);
    ...
    err = spacemit_pmic_i2c_write_unlocked(pmic, reg, val);
        ...
        unsigned mask, unsigned data)
{
    ...
    old = (rt_int32_t)spacemit_pmic_i2c_read_unlocked(pmic, reg);
    ...
    err = spacemit_pmic_i2c_write_unlocked(pmic, reg, tmp);
...
static rt_err_t spacemit_pmic_i2c_probe(struct rt_i2c_client *client)
{
    ...
    if (!(pmic->io_lock = rt_mutex_create("pmic-io", RT_IPC_FLAG_PRIO)))
    {
        rt_free(pmic);
        return -RT_ENOMEM;
    }
    ...
    if (mfd_data->irqchip)
    {
        ...
        /* Match regmap-irq: mask and acknowledge all defined interrupts. */
        for (int i = 0; i < irqchip->num_regs; ++i)
        {
            rt_uint32_t status, valid_mask;

            valid_mask = spacemit_pmic_irq_valid_mask(irqchip, i);
            err = spacemit_pmic_update_bits(pmic,
                    irqchip->mask_base + i * irqchip->reg_stride,
                    valid_mask,
                    irqchip->mask_unmask_non_inverted ? 0 : valid_mask);

            if (err)
            {
                LOG_E("Mask IRQs failed error = %s", rt_strerror(err));
                rt_pic_cancel_irq(&irqchip->parent);
                goto _fail;
            }

            status = spacemit_pmic_read(pmic,
                    irqchip->status_base + i * irqchip->reg_stride);

            if ((rt_err_t)status < 0)
            {
                err = (rt_err_t)status;
                LOG_E("Read IRQ status failed error = %s", rt_strerror(err));
                rt_pic_cancel_irq(&irqchip->parent);
                goto _fail;
            }

            if (irqchip->init_ack_masked && (status & valid_mask))
            {
                err = spacemit_pmic_write(pmic,
                        irqchip->ack_base + i * irqchip->reg_stride,
                        status & valid_mask);

                if (err)
                {
                    LOG_E("ACK masked IRQs failed error = %s", rt_strerror(err));
                    rt_pic_cancel_irq(&irqchip->parent);
                    goto _fail;
                }
            }
        ...
        if ((err = rt_sem_init(&irqchip->irq_sem, "pmic-irq", 0, RT_IPC_FLAG_FIFO)))
        {
            rt_pic_cancel_irq(&irqchip->parent);
            goto _fail;
        }
