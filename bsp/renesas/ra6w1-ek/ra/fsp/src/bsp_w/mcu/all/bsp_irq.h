 ...
 * Macro definitions
 **********************************************************************************************************************/
#define BSP_IRQ_VECTOR_MAX_ENTRIES     (BSP_VECTOR_TABLE_MAX_ENTRIES - BSP_CORTEX_VECTOR_TABLE_ENTRIES)
 ...
 **********************************************************************************************************************/
extern void * gp_renesas_isr_context[BSP_IRQ_VECTOR_MAX_ENTRIES];
 ...
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void R_BSP_IrqStatusClear (IRQn_Type irq)
{
#if BSP_FEATURE_BSP_HAS_ICU

    /* Clear the IR bit in the selected IELSR register. */
    volatile uint32_t * ielsr_reg = (&(ICU->ICU_IELSR0_REG) + irq);
    *ielsr_reg &= ~ICU_ICU_IELSR0_REG_ICU_IR_Msk;

    /* Read back the IELSR register to ensure that the IR bit is cleared. */
    FSP_REGISTER_READ(ielsr_reg);
#else
    (void) irq;
#endif
}
 ...
 * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void R_BSP_IrqClearPending (IRQn_Type irq)
{
    /* Clear the IR bit in the selected IELSR register. */
...
/*******************************************************************************************************************//**
 * Set the NVIC pending interrupt.
 *
 * @param[in] irq            Interrupt for which to set the pending bit. Note that the enums listed for IRQn_Type are
 *                           only those for the Cortex Processor Exceptions Numbers.
 *
 * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
 **********************************************************************************************************************/
__STATIC_INLINE void R_BSP_IrqSetPending (IRQn_Type irq)
{
    // TIN-TODO: This seems to be used to induce a GPIO_Px_IRQ interrupt after wakeup.
    // But there is no clear test case/use case as to why this is needed in the Tin packs.
    // This is probably something that is needed by a demo application and was included in the BSP.
    // Ported for now only for Tin, and we should remove it later on and sanitize the relevant application.
    uint32_t _irq = (uint32_t) irq;
    NVIC->ISPR[(((uint32_t) irq) >> 5UL)] = (uint32_t) (1UL << (_irq & 0x1FUL));
}
 ...
 * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void R_BSP_IrqEnableNoClear (IRQn_Type const irq)
{
 ...
 * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void R_BSP_IrqEnable (IRQn_Type const irq)
{
 ...
 * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void R_BSP_IrqDisable (IRQn_Type const irq)
{
 ...
 **********************************************************************************************************************/
__STATIC_FORCEINLINE void * R_FSP_IsrContextGet (IRQn_Type const irq)
{
    /* This provides access to the ISR context array defined in bsp_irq.c. This is an inline function instead of
     * being part of bsp_irq.c for performance considerations because it is used in interrupt service routines. */
