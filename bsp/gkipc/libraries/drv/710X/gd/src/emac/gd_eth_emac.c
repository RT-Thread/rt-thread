...
void ETH_MAC_RxDataProcess(GD_HANDLE handle)
{
    ...
    while((p->des0 & ETH_DES_R0_OWN) != ETH_DES_R0_OWN)
    {
        ...
        //else
        if(((p->des0) & (ETH_DES_R0_FS | ETH_DES_R0_LS)) == (ETH_DES_R0_FS | ETH_DES_R0_LS))
        {
            ...
            revbuf=(volatile U8*)(p->des2);

            //des=(U8*)(device->NetReceiveBuffer);//give up two bytes for 4 bytes aglin
#if !STRIP_CRC
        ...
        p->des1 = device->SizePerRxBuffer - 1;
        p->des2 = (U32)device->Rxbufferstart + device->CurRxDesSite * device->SizePerRxBuffer;
        p->des3 = 0x0;

        if(device->CurRxDesSite == (device->RxbufferNum - 1))
        {
            p->des1 |= ETH_DES_R1_RER;//PER=1,bit 25,last rxdes
        }
